...
- **Record visibility, permissions, roles, RLS, shared-data queries, or provisioning:** read [shared data access](docs/agent-policies/data-access.md). Verify standard colleagues and related rows, preserve private/restricted data, run the PostgreSQL access regression suite, and check the intended tenant before and after access migrations.
