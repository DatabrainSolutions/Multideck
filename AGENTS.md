...
- Confirm every new UI, notification, email, document, or shared output is classified as Multideck-owned or explicitly tenant-brandable.
- Confirm tenant branding is limited to the allowlist and that authentication, invitations, password resets, verification, access, and security surfaces remain Multideck-branded.
- Confirm every external web surface ignores personal, stored, browser, and system theme preferences and resolves only to a deliberate record override, the configured tenant brand, or the fixed white-and-Multideck-teal fallback.
...
Boost OS gives you a menu of slash commands. Type `/os` to see them all.

Unconditional rules (always apply):

- Read `.agents/memory/project.md` when present, before any work.
- If the user's request has no code surface (hosted platform, external dashboard, manual workflow), say so, then give short numbered steps for where to do it, and do not touch code.
- Preserve user changes; never revert unrelated work.
- Debug from evidence, not assumption.
- For anything else, run `/os` and pick a command — or ask the user which command fits.
