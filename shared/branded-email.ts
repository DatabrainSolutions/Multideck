...
/** Public presentation data only; no backend clients, credentials or environment reads. */
...
export type EmailDocumentDefaults = { bannerUrl: string }
...
export function renderEmailDocument(options: BrandedEmailOptions, defaults: EmailDocumentDefaults) {
  const direction = "ltr"
  const align = "left"
  ...
  const bodyHtml = structuredBody?.html ?? options.body
    ...
    .join("")
  const actionUrl = options.buttonUrl ? options.buttonUrl : null
...
<html lang="${options.locale ?? "en"}" dir="${direction}">
  ...
  <body style="margin:0;padding:0;background:${pageBackground};color:${ink};overflow-wrap:anywhere;">
