...
- `AvailabilityPicker`: the one availability surface for booking and rescheduling. A month showing only the days that genuinely have free time, beside the times inside the chosen day, split into morning, afternoon and evening. It never calculates availability — it is given server-checked starts — and it owns the month, the day, the timezone the times are read in, and the loading, empty, selected and keyboard states. A fortnight of stacked time buttons is not a picker; a month a visitor can scan is.
- `VerificationCodeInput`: the six-box one-time code field shared by sign-in and public booking. Focus travels on its own — typing advances, backspace retreats, a pasted code fills every box — and completion fires once per code, so a parent that re-renders mid-submit cannot verify twice.
