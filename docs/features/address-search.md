...
`LocationAutocomplete` supports editable text and multiline addresses, with optional saved address choices. `AddressSearch` populates a structured form for review. Both use the authenticated `address-search` function and public Photon/OpenStreetMap place data.
