...
## Managed charge catalogue reconciliation — 24 September

- Merged the five newer `origin/dev` commits locally, taking the new managed charge-code catalogue and Quote charge editor as the product direction. Retained freight-specific payer and exchange-rate provenance fields, and composed the Booking managed-code response with provisional state and attachment access rather than dropping either behaviour. Existing historical charge codes remain readable; new saves still follow the managed-code rules.
- Local client TypeScript and Vite build passed. Focused managed-charge, Quote and Booking contracts passed after updating a stale Quote-layout assertion. PostgreSQL charge-code and access-regression checks passed. The broad client suite is not green (879/912, 33 failures), so this is not complete product sign-off.
- Signed-in Chrome checks reached editable JQ20023 and JQ20028. The new charge-code selector offered no eligible option; JQ20028 explicitly reported no codes for its Sea Export scope. Read-only JQ20029 V2 retained its historical charge line. The connected development Supabase project has the old `RATE_ChargeCodes` table (one active code) but lacks the new `RATE_ChargeApplicability` table and `RATECharge_ScopeConfigured` column from `20260923120000_charge_catalogue_applicability.sql`. A real new charge save and Quote-to-Booking conversion therefore remain unverified and blocked pending an approved backend release plus suitable catalogue configuration.
- This reconciliation is a local Git checkpoint only. No shared-database migration, Edge Function release, GitHub push, Vercel deployment, external Customs submission, or change to Customs/iCustoms occurred.
