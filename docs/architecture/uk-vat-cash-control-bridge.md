...
The read-only source inventory and exact-decimal arithmetic preview now bind invoice balances and payment events to one Cash period and report opening/additions/closing/payment VAT separately. They block known date anomalies, credits, stale payment reviews, missing event coverage and arithmetic differences. The source inventory checks each inventoried invoice VAT line against its posted journal tax line, tax nominal and legal entity, and detects tax postings on those invoices without an inventoried source line. It requires current, separately approved accounting VAT controls for every accounting period covering the VAT period, with no uncovered or overlapping days. A movement inventory compares all posted tax-labelled and VAT-control lines in those months with current-period invoice tax postings. It reports verified opening-balance lines separately and blocks unexplained movements or partial accounting months. An accepted-history inventory requires contiguous prior Cash periods and connects each earlier payment line to an accepted production submission backed by a receipt or matching HMRC readback. An approved draft or uncertain dispatch does not count. The full VAT-control inventory now reports the opening balance, period movement and closing balance for each VAT nominal, with a deterministic digest. It classifies every historical and current control posting as invoice source, verified opening package or unclassified. The bridge checks the net opening and closing balance against unpaid invoice VAT, prior accepted Cash net due and period Cash VAT. A dedicated two-operator HMRC settlement path now binds an accepted production return, an unmatched GBP bank statement line, and balanced VAT-control/bank postings. The bank reconciliation includes those posted settlements. The Cash bridge recognises a settlement only when its immutable source, bank match and accepted earlier Cash return still agree; unsupported historical lines and Standard-to-Cash transitions remain blocked. No Cash return has been filed through this path yet.

Next, expose the settlement workflow to authorised operators and add a reviewed Standard-to-Cash transition treatment. Add real PostgreSQL cases for credits/refunds, an accepted pre-entry Standard invoice, altered allocation/review and unsupported postings. Immutable Cash event reconciliation records and source-lock triggers exist, but new sign-offs are blocked until the full bridge is checked atomically with them. The review lock must recheck the bridge digest, prior accepted-return history and monthly approval digests inside its transaction and refuse incomplete or stale evidence. Only after this should Cash registration, period setup and the HMRC filing path be enabled.
