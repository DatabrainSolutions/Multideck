...
test("the standalone declaration header keeps its title context separate from responsive actions", () => {
  const headerActions = pageSource.match(/<div className="flex min-w-0 max-w-full flex-wrap items-center justify-start gap-2 lg:justify-end" data-customs-header-actions>([\s\S]*?)\n          <\/div>/u)?.[1] ?? ""
  ...
  assert.match(pageSource, /w-full max-w-full flex-col items-start gap-1\.5[\s\S]*?lg:items-end[\s\S]*?iCustomsProviderReference[\s\S]*?text-start[\s\S]*?lg:text-end[\s\S]*?data-customs-header-actions[\s\S]*?Declaration status[\s\S]*?<SegmentedControl[\s\S]*?<DeclarationFieldVisibilityPopover[\s\S]*?View declaration[\s\S]*?Save draft/u)
...
test("confirmed iCustoms correlation IDs appear in the header and submission context without a fallback ID", () => {
  ...
  assert.match(pageSource, /iCustomsProviderReference \? <p className="max-w-full text-start[\s\S]*?lg:text-end[\s\S]*?iCustoms correlation ID[\s\S]*?dir="ltr"[\s\S]*?\{iCustomsProviderReference\}/u)
