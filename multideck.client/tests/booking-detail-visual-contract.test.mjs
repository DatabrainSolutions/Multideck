...
test("booking details retain the workspace tabs and use the selected full-width flow", () => {
  ...
  assert.match(details, /title="Goods" contentClassName="sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-6"/u)
  assert.match(details, /BookingCargoWiseField label="Goods description" value=\{goodsDescription\}/u)
  assert.match(details, /BookingCargoWiseField label="Known cargo" value=\{knownCargo\}/u)
  assert.match(details, /editCargo\(0, "knownCargo"\)/u)
  assert.match(details, /editCargo\(0, "description"\)/u)
...
test("booking detail controls keep equal widths across responsive columns", () => {
  ...
  assert.match(source, /SelectTrigger aria-label=\{t\(label\)\} className="h-8 w-full min-w-0/u)
})
