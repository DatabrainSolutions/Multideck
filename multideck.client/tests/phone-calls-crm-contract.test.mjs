...
const phrases = await readFile(new URL("../src/i18n/phone-calls-phrases.ts", import.meta.url), "utf8")
...
test("identity matching and inferred actions remain operator-reviewed", () => {
  ...
  assert.match(phrases, /"Only the call's reviewed lead candidates can be approved here\. Identity match remains separate\."[\s\S]*ar:/u)
...
test("the transcript presents one Agent-to-Handler conversation while keeping provenance in audit detail", () => {
  ...
  assert.match(phrases, /"Time not supplied"[\s\S]*ar:/u)
  ...
  assert.match(phrases, /"One chronological transcript from greeting, through transfer, to the handler conversation"[\s\S]*ar:/u)
...
test("confirmed calls appear on their linked Lead and Company records without name-based matching", () => {
  ...
  assert.match(phrases, /"No confirmed calls are linked to this lead yet\."[\s\S]*ar:/u)
...
test("phone-call controls, analytics evidence, and failure states support the language layer", () => {
  assert.match(phrases, /"Phone calls"[\s\S]*ar: "المكالمات الهاتفية"/u)
  assert.match(phrases, /"Missed · declined · voicemail"/u)
  assert.match(phrases, /"Provider-confirmed outcomes"/u)
  assert.match(phrases, /"The overview could not be loaded\."/u)
  assert.match(phrases, /"3CX confirmed"[\s\S]*ar:/u)
  assert.match(phrases, /"Read-only call record"[\s\S]*ar:/u)
  assert.match(phrases, /"Agent"[\s\S]*ar: "الوكيل"/u)
  assert.match(phrases, /"Handler"[\s\S]*ar: "الموظف المسؤول"/u)
  assert.match(phrases, /"Handler conversation begins"[\s\S]*ar:/u)
  assert.match(phrases, /"Show audit detail"[\s\S]*ar:/u)
  assert.match(phrases, /"AI-assisted analysis"[\s\S]*ar:/u)
  assert.match(phrases, /"Analyse call"[\s\S]*ar:/u)
  assert.match(phrases, /"Follow-up completion"[\s\S]*ar:/u)
  assert.match(phrases, /"Live data sources"[\s\S]*ar:/u)
  assert.match(phrases, /"Not connected"[\s\S]*ar:/u)
  assert.match(phrases, /"Twilio confirmed"[\s\S]*ar:/u)
  assert.match(phrases, /"No confirmed Twilio transfers"[\s\S]*ar:/u)
  assert.match(phrases, /"Deduplicated calls and available outcomes"[\s\S]*ar:/u)
...
test("phone calls refresh only while visible and preserve stale data after a background failure", () => {
  ...
  assert.match(phrases, /"The available transcript is shown\. Some timing, speaker or provider-completion detail is not confirmed\."[\s\S]*ar:/u)
  assert.match(page, /disabled=\{detailReviewBusy\}/u)
  assert.match(phrases, /"Last updated"[\s\S]*ar:/u)
})
...
test("phone-call privacy keeps disclosure, consent, and provenance explicit", () => {
  ...
  assert.match(phrases, /"Privacy and consent"[\s\S]*ar:/u)
  assert.match(phrases, /"Conflicting evidence"[\s\S]*ar:/u)
})
