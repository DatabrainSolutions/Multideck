...
const cargoEditor = read("multideck.client/src/components/multideck/quote-details/quote-cargo-editor.tsx")
...
const workspaceStyles = read("multideck.client/src/feature-workspaces.css")
...
test("party selection keeps related and recent organisations above a hairline with manual entry available", () => {
  ...
  assert.match(details, /className="md-party-fields"/u)
  assert.match(workspaceStyles, /\.md-party-fields \{ display: grid;[^\n]*grid-template-columns: repeat\(12, minmax\(0, 1fr\)\)/u)
  assert.match(details, /label="Billing address"[\s\S]{0,220}className="col-span-12"/u)
  assert.match(details, /label="Billing email"[\s\S]{0,260}className="md-party-email"/u)
  assert.match(workspaceStyles, /\.md-party-fields :is\(\.md-party-contact, \.md-party-email, \.md-party-short\) \{ grid-column: span 6; \}/u)
})
...
test("route inputs derive UN/LOCODE while transit and repeat frequency use linked compact controls", () => {
  ...
  assert.match(fields, /<AutoFilledField className="md-location-code" label="UN\/LOCODE"/u)
  assert.doesNotMatch(fields, /Fills automatically/u)
  assert.match(fields, /autoPopulated=\{unlocodeAutoPopulated\}/u)
  assert.match(fields, /onChange=\{\(input\) => onChange\(resolveLinkedLocation\(options, value, "unlocode", input\)\)\}/u)
  assert.doesNotMatch(fields, /CompactCombobox label="UN\/LOCODE"/u)
  assert.match(fields, /className="md-location-fields-grid"/u)
  assert.match(workspaceStyles, /\.md-location-fields-grid \{ display: grid; grid-template-columns: minmax\(0, 1fr\)/u)
...
test("every send-blocking Incoterm and Sea FCL container field is visible in quote details", () => {
  ...
  assert.match(details, /className="md-freight-container-row"/u)
  assert.match(workspaceStyles, /\.md-freight-container-row \{ display: grid; grid-template-columns:/u)
...
test("auto-populated fields animate provenance, remain editable, and return to normal after override", () => {
  ...
  assert.match(autoPopulatedField, /const mirror = element\.cloneNode\(true\)/u)
  assert.match(autoPopulatedField, /reveal\.className = "md-auto-populated-reveal"/u)
  assert.match(autoPopulatedField, /reveal\.animate\(/u)
  assert.match(autoPopulatedField, /interruptionEvents = \["pointerdown", "keydown", "beforeinput", "input", "focus", "scroll"\]/u)
  ...
  assert.match(styles, /\.md-auto-populated-reveal/u)
...
test("autosave updates an existing quote in place without remounting its route", () => {
  ...
  assert.match(saveChanges, /saveQuoteWorkflow\(currentQuoteId, payload, currentVersion\?\.CusQuoteVersion_ID\)/u)
  ...
  assert.match(saveChanges, /if \(saveInFlightRef\.current \|\| transitioning \|\| !workspaceEditable\) return/u)
...
test("goods, cargo characteristics, hazardous details and customs agents remain operational data", () => {
  ...
  assert.match(cargoEditor, /<CompactCombobox[\s\S]{0,160}label="Package type"[\s\S]{0,240}options=\{freightPackageTypeOptions\}/u)
  assert.match(details, /<QuoteCargoEditor lines=\{quote.cargoLines\}/u)
  ...
  assert.match(cargoEditor, /<CargoHandlingEditor\b/u)
  assert.doesNotMatch(details, /<CargoCharacteristicsField\b/u)
...
test("new quote detail fields load and save through the real workspace payload", () => {
  ...
  assert.match(page, /payer:\s*\{[\s\S]*orgId:\s*quote\.customerId \|\| ""/u)

  assert.match(page, /customerTermsSource: replacesHiddenPayer \? customer\?\.name \?\? "" : fact\("customerTermsSource"\) \|\| \(hasPayerTerms \? payerOrganisation\?\.name/u)
  assert.match(page, /terms: replacesHiddenPayer \? payerTerms\?\.terms\?\.trim\(\) \|\| "" : record\.terms\?\.trim\(\) \|\| payerTerms\?\.terms\?\.trim\(\) \|\| ""/u)
  assert.match(page, /customerNotes: replacesHiddenPayer \? payerTerms\?\.notes\?\.trim\(\) \|\| "" : record\.customerNotes\?\.trim\(\) \|\| payerTerms\?\.notes\?\.trim\(\) \|\| ""/u)
  ...
  assert.match(page, /localRef: typeof record\.customerReference === "string" \? record\.customerReference : record\.reference\.trim\(\)/u)
