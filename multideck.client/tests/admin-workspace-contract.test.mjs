...
test("audit screens use the canonical bounded table, refresh automatically and omit summary noise", () => {
  ...
  assert.match(adminPage, /if \(refreshing \|\| controller\.signal\.aborted\) return/)
  assert.match(adminPage, /if \(!document\.hidden\) void refresh\(true\)/)
  assert.match(adminPage, /window\.setInterval\(returned, auditRefreshIntervalMs\)/)
  assert.match(adminPage, /if \(!quiet\) setLoading\(true\)/)
