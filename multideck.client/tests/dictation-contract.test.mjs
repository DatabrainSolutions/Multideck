...
test("the Figma status pill is bottom-centred and morphs one set of shapes through every state", () => {
  assert.match(statusPill, /type DictationStatusPhase = "transcribing" \| "polishing" \| "complete" \| "allowance" \| "error"/)
  assert.match(statusPill, /transcribing: 181/)
  assert.match(statusPill, /polishing: 134/)
  assert.match(statusPill, /complete: 149/)
  assert.match(statusPill, /allowance: 292/)
  ...
  assert.match(statusPill, /isUrgent = phase === "error" \|\| phase === "allowance"/)
  assert.match(statusPill, /role=\{isUrgent \? "alert" : "status"\}/)
  ...
  assert.match(styles, /data-state="allowance"[\s\S]*var\(--md-amber\)[\s\S]*#fde68a/)
  assert.match(controller, /transcription_allowance_reached[\s\S]*allowanceReached \? "allowance" : "error"/)
...
test("the reusable dictation status pill is documented and previewed in the components gallery", () => {
  ...
  assert.match(galleryPage, /<DictationStatusPill[\s\S]*phase=\{previewDictationPhase\}[\s\S]*Contact admin to increase usage[\s\S]*No clear audio detected/)
  assert.match(galleryPage, /"transcribing", "polishing", "complete", "allowance", "error"/)
  assert.match(galleryPage, /"Out of usage"/)
})
