...
test("the detached actions preserve permissions, in-place confirmation and focus return", () => {
  ...
  assert.match(source, /label="Close" icon=\{X\} disabled=\{saving \|\| Boolean\(responding\)\}/)
  assert.match(source, /selection.anchor.focus\(\{ preventScroll: true \}\)/)
  assert.match(source, /tooltipSide = "right"/)
  assert.match(source, /<TooltipContent side=\{tooltipSide\}>\{label\}/)
})
