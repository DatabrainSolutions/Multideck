...
import { AlertCircle, ArrowRight, BarChart3, Pause, Share2, ShieldCheck, Sparkles } from "lucide-react"
  ...
  return (
    <div className={cn("mx-auto flex w-full min-w-0 flex-col gap-[var(--md-page-stack-gap)] px-[var(--md-page-stack-gap)] py-[var(--md-page-section-gap)]", streamWidth)}>
      {messages.map((message) =>
        message.role === "user" ? (
          <div key={message.id} className="ms-auto max-w-[620px] whitespace-pre-wrap rounded-[var(--md-radius-xl)] bg-[var(--md-surface-tint)] px-5 py-4 text-[15px] leading-6 text-[var(--md-ink)]">
            {message.content}
          </div>
...
export function AgentDexterPage() {
  ...
  const historyItems = useMemo<DexterHistoryItem[]>(
    () => conversations.map((conversation) => ({
      ...
      time: new Date(conversation.updatedAt).toLocaleTimeString(language, { hour: "2-digit", minute: "2-digit" }),
    })),
