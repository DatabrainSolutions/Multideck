      ...
      <div className="divide-y divide-[var(--md-line)]">
        ...
        {visibleAccounts.length > 0 && <div className="max-h-[70vh] overflow-auto overscroll-none" tabIndex={0} role="region" aria-label={t("Chart of accounts")}>
          <table className="w-full min-w-[1160px] table-fixed text-[13px]">
            ...
            <colgroup><col className="w-[10%]" /><col className="w-[20%]" /><col className="w-[15%]" /><col className="w-[16%]" /><col className="w-[15%]" /><col className="w-[8%]" /><col className="w-[8%]" /><col className="w-[8%]" /></colgroup>
            <thead className="sticky top-0 z-10 bg-[var(--md-surface-soft)] text-[var(--md-text)] shadow-[var(--md-stroke-bottom)]">
              <tr>{["Account code", "Account name", "Account type", "Report category", "Accounts system mapping hint", "Control account", "Manual posting", "Actions"].map(label => <th key={label} scope="col" className="px-3 py-2 text-left text-[12px] font-medium">{t(label)}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-[var(--md-line)]">
              {visibleAccounts.map(row => <tr key={rowKey(row)} className={row.isActive === false ? "bg-[var(--md-surface-soft)]" : ""}>
                ...
                <td className="px-3 py-2 [&_label]:sr-only"><Field id={`nominal-map-${rowKey(row)}`} label={t("Accounts system mapping hint")} value={text(row.externalMappingHint)} onChange={externalMappingHint => patchRow("nominalAccounts", row, { externalMappingHint })} ltr /></td>
...
function NominalAccountMappingTable({
  ...
  const [search, setSearch] = useState("")
  const [catalogs, setCatalogs] = useState<Record<string, { accounts: Array<{ name: string; account_number?: string; account_name?: string }>; error?: string; loading?: boolean }>>({})
  ...
  const visible = nominals.filter((row) => `${text(row.code)} ${text(row.name)} ${text(row.accountTypeCode)}`.toLowerCase().includes(search.trim().toLowerCase()))
  ...
  >
    <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
      ...
      <span role="status" className="text-[12px] tabular-nums text-[var(--md-subtle)]">{visible.length} {t("of")} {nominals.length} {t("codes")}</span>
    </div>
      ...
      <table className="w-full min-w-[720px] text-left text-[13px]">
        ...
        <tbody className="divide-y divide-[var(--md-line)]">{visible.map((nominal) => <tr key={rowKey(nominal)} className={nominal.isActive === false ? "text-[var(--md-subtle)]" : undefined}>
          ...
          {connections.map((connection) => {
            ...
            const options = catalog?.accounts.map((account) => ({ value: account.name, label: `${account.account_number || account.name} · ${account.account_name || account.name}` })) ?? []
