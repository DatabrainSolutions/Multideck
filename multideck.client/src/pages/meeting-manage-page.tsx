...
export function MeetingManagePage({ token }: { token: string }) {
  ...
  useEffect(() => {
    ...
    setError(null)
    void getManagedMeetingSlots(token, new Date().toISOString(), new Date(Date.now() + 28 * 86_400_000).toISOString()).then((result) => setSlots(result.slots.filter((slot) => slot !== state.meeting.startAt))).catch((reason) => setError(reason instanceof Error ? reason.message : "Available times could not be loaded.")).finally(() => setSlotsLoading(false))
  }, [rescheduling, state])
