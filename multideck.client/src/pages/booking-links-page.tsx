...
function BookingLinkDrawer({ open, value, connections, restoreFocusTo, navigate, onClose, onSaved }: { open: boolean; value: BookingLink | null; connections: CalendarConnection[]; restoreFocusTo?: HTMLElement | null; navigate: (path: string) => void; onClose: () => void; onSaved: (link: BookingLink) => void }) {
  ...
  return <SideDrawer open={open} onClose={onClose} eyebrow="Calendar" title={value ? "Edit booking link" : "New booking link"} icon={Link} restoreFocusTo={restoreFocusTo} width={520}>
    <div className="grid gap-3">
      <SegmentedControl options={drawerSections} value={section} onChange={(next) => { setSection(next); setError(null) }} ariaLabel="Booking link settings" className="w-full [&>button]:flex-1" />
      {section === "Details" ? <div className="grid gap-3">
        <div className="grid gap-4 rounded-[var(--md-radius-xl)] bg-[var(--md-surface)] p-4 shadow-[var(--md-shadow-line)]">
          <label className={fieldLabel}>Name<Input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder="e.g. 30-minute discovery call" className="h-10 rounded-[var(--md-radius-lg)]" /></label>
        ...
        <div className="grid gap-3 rounded-[var(--md-radius-xl)] bg-[var(--md-surface)] p-4 shadow-[var(--md-shadow-line)]">
      ...
      {section === "Form" ? <div className="rounded-[var(--md-radius-xl)] bg-[var(--md-surface)] p-4 shadow-[var(--md-shadow-line)]">
        <div className="flex items-start justify-between gap-4"><div><p className="text-[12px] font-medium text-[var(--md-ink)]">Booking form</p><p className="mt-1 text-[11px] leading-5 text-[var(--md-subtle)]">Ask only for what the meeting needs. Required answers block the booking until filled in.</p></div><p className="shrink-0 text-[10.5px] tabular-nums text-[var(--md-subtle)]">{customCount} question{customCount === 1 ? "" : "s"} · {requiredCount} required</p></div>
        <BookingQuestionBuilder value={questions} onChange={setQuestions} className="mt-3" />
      ...
      {section === "Availability" ? <div className="grid gap-3">
        ...
        <div className="rounded-[var(--md-radius-xl)] bg-[var(--md-surface)] p-4 shadow-[var(--md-shadow-line)]"><p className="text-[12px] font-medium text-[var(--md-ink)]">Changes and cancellations</p><p className="mt-1 text-[11px] leading-5 text-[var(--md-subtle)]">How close to the meeting an attendee may make a self-service change.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className={compactLabel}>Reschedule cut-off<Select value={rescheduleCutoff} onValueChange={setRescheduleCutoff}><SelectTrigger className="h-9 rounded-[var(--md-radius-md)]"><SelectValue /></SelectTrigger><SelectContent>{cutoffOptions.map(([minutes,label]) => <SelectItem key={minutes} value={String(minutes)}>{label}</SelectItem>)}</SelectContent></Select></label><label className={compactLabel}>Cancellation cut-off<Select value={cancellationCutoff} onValueChange={setCancellationCutoff}><SelectTrigger className="h-9 rounded-[var(--md-radius-md)]"><SelectValue /></SelectTrigger><SelectContent>{cutoffOptions.map(([minutes,label]) => <SelectItem key={minutes} value={String(minutes)}>{label}</SelectItem>)}</SelectContent></Select></label></div></div>
      </div> : null}
      ...
      <div className="sticky bottom-0 flex justify-end gap-2 rounded-[var(--md-radius-xl)] bg-[color-mix(in_srgb,var(--md-surface)_94%,transparent)] p-2 shadow-[var(--md-shadow-line)] backdrop-blur-xl"><Button variant="ghost" onClick={onClose} disabled={saving}>Cancel</Button><Button onClick={() => void save()} disabled={saving}>{saving ? "Saving…" : value ? "Save changes" : "Create booking link"}</Button></div>
    </div>
  </SideDrawer>
}
...
export function BookingLinksPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  return <main className="grid gap-[var(--md-page-stack-gap)]"><section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-medium uppercase tracking-[.08em] text-[var(--md-subtle)]">Calendar</p><h1 className="mt-1 text-[24px] font-medium tracking-[-.02em] text-[var(--md-ink)]">Booking links</h1><p className="mt-1 max-w-2xl text-[13px] leading-6 text-[var(--md-text)]">Reusable personal meeting types that only offer times you are genuinely free.</p></div><Button variant="ghost" onClick={() => navigate("/calendar")} className="h-9 rounded-[var(--md-radius-md)] bg-[var(--md-surface)] shadow-[var(--md-shadow-line)]"><CalendarDays className="size-3.5" />Back to Calendar</Button></section>
    ...
    <BookingLinkDrawer open={drawerOpen} value={editing} connections={connections} restoreFocusTo={restoreFocusRef.current} navigate={navigate} onClose={() => setDrawerOpen(false)} onSaved={upsert} />
