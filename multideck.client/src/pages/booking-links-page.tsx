...
export function BookingLinksPage({ navigate, embedded = false }: { navigate: (path: string) => void; embedded?: boolean }) {
  ...
  return <Container className="@container grid gap-[var(--md-page-stack-gap)]">{!embedded ? <section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[11px] font-medium uppercase tracking-[.08em] text-[var(--md-subtle)]">Calendar</p><h1 className="mt-1 text-[24px] font-medium tracking-[-.02em] text-[var(--md-ink)]">Booking links</h1><p className="mt-1 max-w-2xl text-[13px] leading-6 text-[var(--md-text)]">Reusable personal meeting types that only offer times you are genuinely free.</p></div><Button variant="ghost" onClick={() => navigate("/calendar")} className="h-9 rounded-[var(--md-radius-md)] bg-[var(--md-surface)] shadow-[var(--md-shadow-line)]"><CalendarDays className="size-3.5" />Back to Calendar</Button></section> : null}
    {loading ? <div className="grid min-h-72 place-items-center rounded-[var(--md-radius-xl)] bg-[var(--md-surface)] shadow-[var(--md-shadow-line)]"><DotGridLoader label="Loading booking links…" /></div> : error ? <div role="alert" className="grid min-h-64 place-items-center rounded-[var(--md-radius-xl)] bg-[var(--md-surface)] p-8 text-center shadow-[var(--md-shadow-line)]"><div><TriangleAlert className="mx-auto size-5 text-[var(--md-red)]" /><p className="mt-3 text-[12px] text-[var(--md-text)]">{error}</p><Button className="mt-4" onClick={load}>Try again</Button></div></div> : !links.length ? <div className="grid min-h-72 place-items-center rounded-[var(--md-radius-xl)] bg-[var(--md-surface)] p-8 text-center shadow-[var(--md-shadow-line)]"><div className="max-w-sm"><EmptyStateIllustration variant="calendar" /><p className="mt-4 text-[14px] font-medium text-[var(--md-ink)]">No booking links yet</p><p className="mt-2 text-[12px] leading-5 text-[var(--md-subtle)]">Create a meeting type once, then share a company-branded link whenever someone needs time with you.</p><Button className="mt-5" onClick={startCreate}><Plus className="size-4" />New booking link</Button></div></div> : <div className="overflow-hidden rounded-[var(--md-radius-xl)] bg-[var(--md-surface)] shadow-[var(--md-shadow-line)]"><div className="hidden grid-cols-[minmax(0,1fr)_150px_90px_150px_120px_310px] gap-3 border-b border-[var(--md-line)] px-4 py-3 text-[10px] font-medium uppercase tracking-[.06em] text-[var(--md-subtle)] @min-[1080px]:grid"><span>Booking link</span><span>Hosts</span><span>Duration</span><span>Provider</span><span>Status</span><span className="text-end">Actions</span></div><div className="divide-y divide-[var(--md-line)]">{links.map((link) => <article key={link.id} className="grid gap-3 px-4 py-4 transition-colors hover:bg-[var(--md-hover)] @min-[1080px]:grid-cols-[minmax(0,1fr)_150px_90px_150px_120px_310px] @min-[1080px]:items-center"><div className="min-w-0"><p className="truncate text-[13px] font-medium text-[var(--md-ink)]">{link.title}</p><p className="mt-1 truncate text-[10.5px] text-[var(--md-subtle)]">{link.path}</p></div><div className="min-w-0 text-[11px] text-[var(--md-text)]"><p className="truncate">{bookingLinkKindLabels[link.kind].label}</p>{link.kind !== "one_on_one" ? <p className="mt-0.5 truncate text-[10.5px] text-[var(--md-subtle)]">{link.hosts.length} host{link.hosts.length === 1 ? "" : "s"} · {link.hosts.map((host) => host.name.split(" ")[0]).join(", ")}</p> : null}</div><p className="text-[11px] tabular-nums text-[var(--md-text)]">{link.durationMinutes} min</p><div className="flex items-center gap-2 text-[11px] text-[var(--md-text)]"><MeetingProviderMark provider={link.provider} className="size-4" />{meetingProviderLabels[link.provider]}</div><span className={cn("w-fit rounded-full px-2 py-1 text-[10px] font-medium capitalize", link.status === "active" ? "bg-[var(--md-accent-a10)] text-[var(--md-accent)]" : "bg-[var(--md-surface-tint)] text-[var(--md-subtle)]")}>{link.status}</span><div className="flex flex-wrap justify-start gap-1.5 @min-[1080px]:justify-end"><Button variant="ghost" size="sm" onClick={() => void copy(link)} className="h-8 rounded-[var(--md-radius-md)]"><Copy className="size-3.5" />Copy link</Button><Button variant="ghost" size="sm" onClick={() => navigate(link.path)} className="h-8 rounded-[var(--md-radius-md)]"><Eye className="size-3.5" />Preview</Button><Button variant="ghost" size="sm" onClick={() => openDrawer(link)} className="h-8 rounded-[var(--md-radius-md)]">Edit</Button><Button variant="ghost" size="icon" aria-label={link.status === "active" ? "Pause booking link" : "Activate booking link"} onClick={() => void toggle(link)} className="size-8 rounded-[var(--md-radius-md)]">{link.status === "active" ? <Pause className="size-3.5" /> : <Play className="size-3.5" />}</Button><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label="More booking link actions" className="size-8 rounded-[var(--md-radius-md)]"><MoreHorizontal className="size-3.5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48"><DropdownMenuItem variant="destructive" onSelect={() => setDeleteTarget(link)}><Trash2 className="size-3.5" />Delete booking link</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div></article>)}</div></div>}
