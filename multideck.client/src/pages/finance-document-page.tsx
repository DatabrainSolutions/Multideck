...
function providerLabel(code: string | undefined) {
  ...
  return code?.replaceAll("_", " ") || "Accounts system"
}
...
export function FinanceDocumentPage({
  ...
  const vatSourceLocked = Boolean(detail?.vatReconciliation?.sourceLocked)
  const editable = Boolean(document?.FINDoc_StatusCode === "draft" && !vatSourceLocked && canDraft && options)
  ...
  return (
    <>
      ...
      <div className="mt-[var(--md-page-stack-gap)] space-y-[var(--md-page-stack-gap)]">
        ...
        {posted && detail.vatReconciliation ? <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 px-1 text-[12px] text-[var(--md-text)]">
          <span className="font-medium text-[var(--md-ink)]">{t(detail.vatReconciliation.status === "reconciled" ? "VAT reconciled" : "VAT transaction sign-off")}</span>
          {detail.vatReconciliation.status === "reconciled" && detail.vatReconciliation.vatReconciledAt
            ? <time dateTime={detail.vatReconciliation.vatReconciledAt} data-i18n-skip dir="ltr">{dateFormatter.format(new Date(detail.vatReconciliation.vatReconciledAt))}</time>
            : <span>{t("Pending sign-off")} · <span data-i18n-skip dir="ltr">{detail.vatReconciliation.reconciledLines}/{detail.vatReconciliation.totalLines}</span> {t("lines")}</span>}
          {detail.vatReconciliation.status !== "reconciled" && detail.vatReconciliation.firstCompleteVatReconciledAt ? <span className="text-[var(--md-subtle)]">· {t("First fully VAT reconciled")} <time dateTime={detail.vatReconciliation.firstCompleteVatReconciledAt} data-i18n-skip dir="ltr">{dateFormatter.format(new Date(detail.vatReconciliation.firstCompleteVatReconciledAt))}</time></span> : null}
          {detail.vatReconciliation.sourceLocked ? <span className="text-[var(--md-subtle)]">· {t("Source transaction locked; correct with a new record")}</span> : null}
        </div> : null}
      ...
      <Dialog open={Boolean(reasonAction)} onOpenChange={(open) => { if (!open && !pendingAction) { setReasonAction(null); setReason("") } }}>
        <DialogContent className="sm:max-w-[540px]">
          <DialogHeader><DialogTitle>{t(reasonAction === "reject" ? "Reject finance document" : "Return document to draft")}</DialogTitle><DialogDescription>{t(reasonAction === "reject" ? "Record why this document cannot be approved. The reason is retained in its audit history." : "This revokes the prior approval and cancels the blocked accounts system queue item before editing is enabled.")}</DialogDescription></DialogHeader>
