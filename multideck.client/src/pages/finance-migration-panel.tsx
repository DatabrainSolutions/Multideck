...
export function FinanceMigrationPanel({ entityId, baseCurrency, chartDirty, canDeliverOpeningMirror = false, canPrepareFX = false, canPostFX = false }: { entityId: string; baseCurrency: string; chartDirty: boolean; canDeliverOpeningMirror?: boolean; canPrepareFX?: boolean; canPostFX?: boolean }) {
  ...
  const stageOpening = async () => {
    ...
    try {
      ...
      setPackageNotice(t("Opening balances staged. Review and approve them before posting. Your entity policy may require another finance operator."))
      await loadPackages()
  ...
  const settleFX = async (row: OpeningFXWorkItem, action: "propose" | "post") => {
    ...
    try {
      ...
      setFxNotice(t(action === "propose" ? "Settlement proposed. Review and post it; your entity policy may require another operator." : "Settlement posted to the native ledger."))
  ...
  return <SettingsPanel title={t("CargoWise opening balances")} description={t("Upload a closing trial balance and open transactions, review the source columns, then reconcile them to this legal entity's saved chart.")}>
    <details className="px-5 py-4"><summary className="cursor-pointer text-[13px] font-medium text-[var(--md-accent)] focus-visible:outline-2 focus-visible:outline-[var(--md-accent)]">{t("Prepare migration preview")}</summary>
      <div className="mt-4 space-y-5">
        <p className="text-xs text-[var(--md-text)]">{t("The preview does not save files. It sends converted balances and transactions to your tenant's finance service. Reconciled source rows can be staged, reviewed and posted to an empty native ledger under the entity approval policy.")}</p>
    ...
    <div className="space-y-3 border-t border-[var(--md-line)] px-5 py-4">
      ...
      {fxPackageId && <section className="space-y-3 border-t border-[var(--md-line)] pt-4" aria-label={t("Opening cash settlements")}>
        ...
        <p className="text-xs text-[var(--md-text)]">{t("Compare the cash amount at the bank rate with the imported invoice carrying value. An operator posts the control reclassification and realised FX adjustment under the entity approval policy.")}</p>
