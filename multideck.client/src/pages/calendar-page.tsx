...
export function CalendarPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const events = useMemo(() => [...(workspace?.meetings ?? []), ...(workspace?.externalEvents ?? []), ...(workspace?.companyEvents ?? [])], [workspace])
