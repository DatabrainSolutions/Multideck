...
import { getCalendarWorkspace, updateMeeting, type CalendarEvent, type CalendarWorkspace } from "@/lib/calendar-api"
...
export function CalendarPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  async function confirmMove() {
    ...
    try {
      const result = await updateMeeting(pendingMove.event.id, { startAt: pendingMove.startAt, endAt: pendingMove.endAt, timeZone: pendingMove.event.timeZone })
      toast.success(result.status === "sync_pending" ? "Provider update requested" : "Meeting rescheduled", { description: result.status === "sync_pending" ? "The current time remains confirmed until the provider accepts the change." : "Attendees will receive the updated time." })
      setPendingMove(null); load()
    } catch (reason) {
      setMoveError(reason instanceof Error ? reason.message : "The meeting could not be rescheduled.")
    } finally { setMoving(false) }
  }
  const moveDate = pendingMove ? new Intl.DateTimeFormat("en-GB", { weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit", timeZone: pendingMove.event.timeZone }).format(new Date(pendingMove.startAt)) : ""
  return <main className="grid gap-[var(--md-page-stack-gap)]">
    ...
    <Dialog open={Boolean(pendingMove)} onOpenChange={(open) => { if (!open && !moving) { setPendingMove(null); setMoveError(null) } }}><DialogContent className="rounded-[var(--md-radius-xl)] sm:max-w-[430px]"><DialogHeader><DialogTitle>Review new meeting time</DialogTitle><DialogDescription>Moving a meeting affects attendees. Nothing changes until you confirm.</DialogDescription></DialogHeader>{pendingMove ? <div className="rounded-[var(--md-radius-lg)] bg-[var(--md-surface-tint)] px-4 py-3"><p className="text-[12px] font-medium text-[var(--md-ink)]">{pendingMove.event.title}</p><p className="mt-1 text-[12px] text-[var(--md-text)]">Move to {moveDate}</p><p className="mt-1 text-[11px] text-[var(--md-subtle)]">{pendingMove.event.timeZone}</p></div> : null}{moveError ? <p role="alert" className="text-[12px] text-[var(--md-red)]">{moveError}</p> : null}<DialogFooter><Button type="button" variant="ghost" disabled={moving} onClick={() => { setPendingMove(null); setMoveError(null) }}>Keep current time</Button><Button type="button" disabled={moving} onClick={() => void confirmMove()}>{moving ? "Updating…" : "Confirm new time"}</Button></DialogFooter></DialogContent></Dialog>
  </main>
}
