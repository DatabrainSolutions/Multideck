...
import { customsToday, hmrcMonthlyRatesUrl, fetchHmrcMonthlyRates, selectHmrcExchangeRate, validCustomsConversionDate } from "../../../supabase/functions/_shared/customs-hmrc-exchange-rates.mts"
  ...
  return <SectionFrame title={t("Declaration details")} description={t("Set up the declaration, then confirm references and goods location.")}>
    ...
    <section aria-labelledby="customs-declaration-setup-heading">
      ...
      <FieldShell label="DUCR" required showDataElements={showDataElements} fieldKey="ducr" invalid={Boolean(ducrError)} highlighted={highlightedField === "ducr"} notice={!eori ? t("Configure the registered company or office EORI in Admin → System Preferences → Customs preferences.") : undefined}>
        <span ref={ducrRef} className="block"><Input aria-label="DUCR" value={draft.ducr} aria-required="true" aria-invalid={Boolean(ducrError)} aria-describedby={ducrError ? "customs-ducr-error" : "customs-ducr-help"} onChange={(event) => { setPopulationEvent(null); update("autoGeneratedDucr", undefined); update("ducr", event.target.value.toUpperCase()) }} className={customsSingleLineControlClass} /></span>
    ...
    <section aria-labelledby="customs-declaration-identifiers-heading">
      ...
      <FieldShell label={t("Status")} showDataElements={showDataElements}><Input readOnly value={t(titleCase(customsState?.declaration.provider?.status ?? customsState?.declaration.status ?? "draft"))} className={customsSingleLineControlClass} /></FieldShell>
      </FieldGrid>
    </section>
    </div>
...
function AuthorisationHoldersTable({ draft, update, showDataElements, issues, highlightedField, t }: SectionProps) {
  ...
  return <section aria-labelledby="authorisation-holders-heading" className="col-span-full min-w-0">
    <div className="mb-3 flex items-center gap-2"><h4 id="authorisation-holders-heading" className="text-[13px] font-medium text-[var(--md-ink)]">{t("Authorisation holders")}</h4>{showDataElements ? <span className="text-[10px] text-[var(--md-blue)]">DE 3/39</span> : null}{showBoxes ? <span className="text-[10px] text-[var(--md-accent)]">Box 44</span> : null}</div>
...
function ImportTaxPartiesTable({ draft, update, showDataElements, issues, highlightedField, t }: SectionProps) {
  ...
  return <section aria-labelledby="customs-import-tax-parties-heading" data-customs-field="domesticDutyTaxParties" className="min-w-0 rounded-[var(--md-radius-lg)] bg-[var(--md-surface)] p-4 shadow-[var(--md-shadow-line)]">
    <div className="mb-3 flex items-center gap-2"><h3 id="customs-import-tax-parties-heading" className="text-[13px] font-medium text-[var(--md-ink)]">{t("Domestic duty tax parties")}</h3>{showDataElements ? <span className="text-[10px] text-[var(--md-blue)]">DE 3/40</span> : null}</div>
    <p className="mb-2 text-[11px] text-[var(--md-subtle)] sm:hidden">{t("Scroll across the table to see every field.")}</p>
...
function ImportLoadingLocationField({ draft, update, showDataElements, issues, highlightedField, t }: SectionProps) {
  ...
  return <div data-customs-field="loadingLocationId" className={cn("min-w-0 basis-full rounded-[var(--md-radius-md)] sm:basis-[200px] sm:grow", highlightedField === "loadingLocationId" && "ring-2 ring-[var(--md-accent)]")}>
    <CompactCombobox label={t("Loading location ID") + (showDataElements ? " · DE 5/21" : "")} value={draft.loadingLocationId ?? ""} options={options} onValueChange={value => update("loadingLocationId", value.toUpperCase().slice(0, 35))} onOptionSelect={option => update("loadingLocationId", option.value)} placeholder={t(loading ? "Loading airports — or type a code" : "Search airport or type a code")} allLabel={t("IATA airports")} emptyLabel={t(failed ? "Airport list unavailable — enter a code manually" : "No matching airport — use your own code")} width="full" invalid={issues.has("loadingLocationId")} className="[&_input]:h-8 [&_input]:text-[11px]" />
  </div>
}
...
function ImportGuaranteesTable({ draft, update, showDataElements, issues, highlightedField, t }: SectionProps) {
  ...
  return <section aria-labelledby="customs-guarantees-heading" data-customs-field="guarantees" className="min-w-0 rounded-[var(--md-radius-lg)] bg-[var(--md-surface)] p-4 shadow-[var(--md-shadow-line)]">
    <div className="mb-3 flex items-center gap-2"><h3 id="customs-guarantees-heading" className="text-[13px] font-medium text-[var(--md-ink)]">{t("Guarantees")}</h3>{showDataElements ? <span className="text-[10px] text-[var(--md-blue)]">DE 8/2 · 8/3</span> : null}</div>
    <p className="mb-2 text-[11px] text-[var(--md-subtle)] sm:hidden">{t("Scroll across the table to see every field.")}</p>
...
function InvoiceHeadersSection({ draft, onChange, onConversionDateChange, onImport, t }: { draft: StandaloneExportDraft; onChange: (headers: CustomsInvoiceHeader[]) => void; onConversionDateChange: (date: string) => void; onImport?: () => void; t: (text: string) => string }) {
  ...
  return <Surface padding="none" className="min-w-0 overflow-hidden rounded-[var(--md-radius-xl)]">
    <header className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--md-line)] px-4 py-3">
      <div className="min-w-0"><h2 className="text-[14px] font-medium text-[var(--md-ink)]">{t("Invoice header")}</h2><p className="mt-1 max-w-2xl text-[11px] leading-5 text-[var(--md-text)]">{t("Add each commercial invoice, then choose its number on the invoice items. Renaming an invoice keeps its items linked.")}</p><p className="text-[11px] leading-5 text-[var(--md-subtle)]">{t("Choose, reorder and pin columns to suit your work. Header details belong to each invoice; complete the amount and currency before submission. Move linked items before removing a header.")}</p></div>
    ...
    <div className="flex flex-wrap items-end gap-3 border-b border-[var(--md-line)] px-4 py-3">
      <div><label htmlFor="customs-conversion-date" className="mb-1 block text-[11px] font-medium">{t("Estimate date")}</label><Input id="customs-conversion-date" type="date" value={conversionDate} readOnly className={cn(customsSingleLineControlClass, "w-[165px]")} /></div>
      <Button type="button" variant="outline" size="sm" disabled={!currencyKey || !validCustomsConversionDate(conversionDate) || ratesBusy} onClick={() => { setRefreshDate(customsToday()); setRatesRefresh(value => value + 1) }}>{ratesBusy ? <DotGridLoader className="size-3.5" /> : <RefreshCw className="size-3.5" />}{t("Refresh HMRC rates")}</Button>
      <p className="max-w-xl text-[11px] leading-5 text-[var(--md-text)]">{t("This is an estimate using HMRC rates for the date shown. Refresh to today’s rates before sharing an updated estimate. Rates are checked again for the submission date before you confirm.")}</p>
    </div>
    ...
    {errors.length ? <ul className="space-y-1 border-t border-[var(--md-line)] px-4 py-3 text-[11px] text-[var(--md-red)]" aria-label={t("Invoice header checks")}>{errors.map(error => <li key={error.field}>{t(error.message)}</li>)}</ul> : null}
  </Surface>
}
...
function FieldShell({ label, dataElement, customsBox, required, showDataElements, invalid, highlighted, fieldKey, className, errorId, notice, children }: { label: string; dataElement?: string; customsBox?: string; required?: boolean; showDataElements: boolean; invalid?: boolean; highlighted?: boolean; fieldKey?: string; className?: string; errorId?: string; notice?: string; children: ReactNode }) {
  ...
  const field = <div data-customs-field-shell data-compact={compact || undefined} data-field-invalid={invalid || undefined} data-has-notice={notice || undefined} className={cn("relative min-w-0", className)}><label className={cn("block min-w-0", compact && "grid grid-cols-[minmax(76px,0.42fr)_minmax(0,0.58fr)] items-center gap-1.5")}><span className={cn("flex items-center gap-1.5 font-medium text-[var(--md-text)]", compact ? "min-h-0 text-[10.5px] leading-[1.15]" : "mb-1.5 min-h-5 text-[11px]")}><span className={cn(compact ? "line-clamp-2" : "truncate")}>{label}</span>{required ? <span className="text-[var(--md-red)]">*</span> : null}{showAnnotations ? <span className="flex shrink-0 items-center gap-1">{showDataElements && dataElement ? <span className={cn("rounded-[var(--md-radius-sm)] bg-[color-mix(in_srgb,var(--md-blue)_8%,transparent)] font-medium tabular-nums text-[var(--md-blue)]", compact ? "px-1 py-0.5 text-[8.5px]" : "px-1.5 py-0.5 text-[10px]")} dir="ltr">DE {dataElement}</span> : null}{showCustomsBoxNumbers && customsBox ? <span className={cn("rounded-[var(--md-radius-sm)] bg-[var(--md-accent-a10)] font-medium tabular-nums text-[var(--md-accent)]", compact ? "px-1 py-0.5 text-[8.5px]" : "px-1.5 py-0.5 text-[10px]")} dir="ltr">{`Box ${customsBox}`}</span> : null}</span> : null}</span><span data-customs-field={fieldKey} className={cn("block rounded-[var(--md-radius-md)] transition-[box-shadow] duration-300", highlighted && "ring-2 ring-[var(--md-accent)] shadow-[0_0_20px_var(--md-accent)]")}>{children}</span>{error ? <span id={errorId} aria-hidden={errorId ? true : undefined} className="customs-field-error"><CircleAlert className="size-3.5 shrink-0" aria-hidden="true" />{error}</span> : null}</label>{noticeControl}</div>
...
function IncotermsLocationField({ value, onChange, showDataElements, invalid, highlighted, t }: {
  ...
  const field = <div data-customs-field="tradeTermsLocation" className={cn("min-w-0 basis-full rounded-[var(--md-radius-md)] transition-[box-shadow] duration-300 sm:basis-[180px] sm:grow", highlighted && "ring-2 ring-[var(--md-accent)] shadow-[0_0_20px_var(--md-accent)]")}>
    <div className="relative">
      <CompactCombobox
        ...
        width="full"
        valueDirection="ltr"
        className={cn((showDataElements || showCustomsBoxNumbers) && "[&>div:first-child]:pe-32", "grid-cols-1 [&_input]:h-8 [&_input]:text-[11px]")}
      />
      {showDataElements || showCustomsBoxNumbers ? <span className="pointer-events-none absolute end-0 top-0 flex items-center gap-1">{showDataElements ? <span className="rounded-[var(--md-radius-sm)] bg-[color-mix(in_srgb,var(--md-blue)_8%,transparent)] px-1.5 py-0.5 text-[10px] font-medium tabular-nums text-[var(--md-blue)]" dir="ltr">DE 4/1</span> : null}{showCustomsBoxNumbers ? <span className="rounded-[var(--md-radius-sm)] bg-[var(--md-accent-a10)] px-1.5 py-0.5 text-[10px] font-medium tabular-nums text-[var(--md-accent)]" dir="ltr">Box 20</span> : null}</span> : null}
    </div>
  </div>
  return renderCustomsFieldTooltip(field, t("Incoterms additional"), showCustomsBoxNumbers ? customsFieldHelp("Incoterms additional", "4/1") : undefined)
}
