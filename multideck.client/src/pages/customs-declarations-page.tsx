...
const customsSingleLineControlClass = "h-8 rounded-[var(--md-radius-md)] px-2 text-[11px]"
const customsMultilineControlClass = "rounded-[var(--md-radius-md)]"
  ...
  return <SectionFrame title={t("Declaration details")} description={t("Core identity and references for this import declaration.")}>
    <FieldGrid>
      ...
      <FieldShell label="DUCR" required showDataElements={showDataElements} fieldKey="ducr" invalid={Boolean(ducrError)} highlighted={highlightedField === "ducr"}>
        <span ref={ducrRef} className="block"><Input value={draft.ducr} aria-required="true" aria-invalid={Boolean(ducrError)} aria-describedby={ducrError ? "customs-ducr-error" : "customs-ducr-help"} onChange={(event) => { setPopulationEvent(null); update("autoGeneratedDucr", undefined); update("ducr", event.target.value.toUpperCase()) }} className={customsSingleLineControlClass} /></span>
      </FieldShell>
...
function TextAreaField({ label, value, onChange, dataElement, customsBox, required, showDataElements, invalid, highlighted, fieldKey, className }: { label: string; value: string; onChange: (value: string) => void; dataElement?: string; customsBox?: string; required?: boolean; showDataElements: boolean; invalid?: boolean; highlighted?: boolean; fieldKey?: string; className?: string }) {
  const compact = useContext(CompactCustomsFormContext)
  return <FieldShell label={label} dataElement={dataElement} customsBox={customsBox} required={required} showDataElements={showDataElements} invalid={invalid} highlighted={highlighted} fieldKey={fieldKey} className={className}><Textarea value={value} onChange={(event) => onChange(event.target.value)} aria-invalid={invalid || undefined} className={cn("border-0 bg-[var(--md-field-bg)] shadow-[var(--md-shadow-line)]", customsMultilineControlClass, compact ? "min-h-8 px-2 py-1.5 text-[11px]" : "min-h-9 text-[13px]")} /></FieldShell>
}
