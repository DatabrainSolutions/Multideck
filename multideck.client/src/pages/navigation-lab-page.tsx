  ...
  return (
    <div className="mx-auto w-full max-w-[1480px] py-[var(--md-page-section-gap)]">
      <div className="flex flex-wrap items-start justify-between gap-[var(--md-gap-lg)]">
        ...
        <Badge variant="outline" className="h-7 rounded-[var(--md-radius-md)] border-0 bg-[rgba(14,125,116,0.1)] px-2.5 text-[12px] text-[var(--md-accent)]">{t("Prototype - not connected")}</Badge>
      </div>
      ...
      <div className="mt-[var(--md-page-section-gap)] grid gap-[var(--md-gap-lg)] xl:grid-cols-[minmax(0,1fr)_320px]">
        <Surface padding="md" className="rounded-[var(--md-radius-xl)]"><SectionHeader title={t("Menu structure")} meta={t(`${optionCount} options visible for this review`)} action={<span className="text-[12px] text-[var(--md-subtle)]">{t("Current role")}: {t(role)}</span>} />
          {favourites.length ? <div className="mt-[var(--md-gap-xl)]"><p className="mb-2 flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-[0.08em] text-[var(--md-subtle)]"><Star className="size-3" strokeWidth={1.6} />{t("Favourites")}</p><div className="flex flex-wrap gap-2">{favourites.map((item) => { const Icon = item.icon; return <button key={item.label} type="button" onClick={() => selectItem(item.label)} className={cn("flex h-9 items-center gap-2 rounded-[var(--md-radius-md)] bg-[var(--md-surface-tint)] px-3 text-[12px] font-medium text-[var(--md-text)] shadow-[var(--md-shadow-line)] transition hover:bg-[rgba(14,125,116,0.1)] hover:text-[var(--md-accent)]", selectedLabel === item.label && "bg-[rgba(14,125,116,0.12)] text-[var(--md-accent)]")}><Icon className="size-3.5" strokeWidth={1.4} />{t(item.label)}</button> })}</div></div> : null}
          <div className="mt-[var(--md-gap-xl)] divide-y divide-[var(--md-line)] border-y border-[var(--md-line)]">{groups.map((group) => { const GroupIcon = group.icon; const isOpen = openGroups.has(group.label) || Boolean(query.trim()); return <div key={group.label} className="py-1.5"><button type="button" className="flex w-full items-center gap-3 rounded-[var(--md-radius-md)] px-2 py-2.5 text-start transition hover:bg-[var(--md-hover)] focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-[rgba(14,125,116,0.14)]" aria-expanded={isOpen} onClick={() => toggleGroup(group.label)}><span className="grid size-8 place-items-center rounded-[var(--md-radius-md)] bg-[var(--md-surface-tint)] text-[var(--md-text)]"><GroupIcon className="size-4" strokeWidth={1.35} /></span><span className="min-w-0 flex-1"><span className="block text-[13px] font-medium text-[var(--md-ink)]">{t(group.label)}</span><span className="mt-0.5 block text-[12px] text-[var(--md-text)]">{t(group.detail)}</span></span><span className="rounded-full bg-[var(--md-surface-tint)] px-2 py-0.5 text-[11px] font-medium text-[var(--md-subtle)]">{group.items.length}</span><ChevronDown className={cn("size-4 text-[var(--md-subtle)] transition-transform", isOpen && "rotate-180")} strokeWidth={1.4} /></button>{isOpen ? <div className="ms-5 mt-1 border-s border-[var(--md-line-strong)] ps-4">{group.items.map((item) => { const ItemIcon = item.icon; return <button key={item.label} type="button" onClick={() => selectItem(item.label)} className={cn("flex w-full items-center gap-3 rounded-[var(--md-radius-md)] px-2 py-2 text-start transition hover:bg-[var(--md-hover)]", selectedLabel === item.label && "bg-[rgba(14,125,116,0.1)]")}><ItemIcon className={cn("size-4 shrink-0 text-[var(--md-subtle)]", selectedLabel === item.label && "text-[var(--md-accent)]")} strokeWidth={1.35} /><span className="min-w-0 flex-1"><span className={cn("block text-[12px] font-medium text-[var(--md-ink)]", selectedLabel === item.label && "text-[var(--md-accent)]")}>{t(item.label)}</span><span className="mt-0.5 block text-[11px] text-[var(--md-text)]">{t(item.detail)}</span></span></button> })}</div> : null}</div> })}</div>
        </Surface>
