...
import { createJournalReversal, getGlEntities, getGlTransaction, getGlWorkspace, journalAction, journalTotal, type GlEntity, type GlEntry, type GlWorkspace, type GlTransaction, type Journal, type JournalLine } from "@/lib/finance-ledger-api"
...
export function FinanceGeneralLedgerPage({ route, navigate, currentUser }: { route: string; navigate: (path: string) => void; currentUser?: AuthUserSummary | null }) {
  ...
  useEffect(() => { let active = true; void getGlEntities().then(result => { if (active) { const saved = rememberedEntity(); const selected = result.entities.some(row => row.LegalEntity_ID === saved) ? saved || "" : result.entities.length === 1 ? result.entities[0].LegalEntity_ID : ""; setEntities(result.entities); setEntity(selected); if (!selected) setLoading(false) } }).catch(cause => { if (active) { setError(cause.message); setLoading(false) } }); return () => { active = false } }, [])
  ...
  const prepareReversal = async () => {
    if (!journal || !canPrepare || busy || reversalReason.trim().length < 3) return
    setBusy(true)
    try {
      const saved = await createJournalReversal(entity, journal.id, reversalReason.trim())
      setJournal(saved); setReversalReason(""); setDirty(false); setConfirmPost(false); setError("")
      setNotice("Linked reversal draft saved. Review its accounting date, then post. The original journal is unchanged.")
      await load()
  ...
  const linesReadonly = readonly || Boolean(journal?.reversal_of_id)
  const importLines = async (file: File) => {
    if (!journal || journal.status !== "draft" || journal.reversal_of_id || !canPrepare || busy || importing || !workspace) return
  ...
  return <>
    ...
    <div className="mt-[var(--md-page-stack-gap)] space-y-[var(--md-page-stack-gap)]">
      ...
      <SettingsPanel title={t("Ledger selection")}><div className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-5">
        <label className="grid gap-1 text-xs">{t("Legal entity")}<select className={selectClass} value={entity} onChange={event => changeEntity(event.target.value)} disabled={busy || importing}><option value="" disabled>{t("Choose legal entity")}</option>{entities.map(row => <option key={row.LegalEntity_ID} value={row.LegalEntity_ID}>{row.LegalEntity_Name}</option>)}</select></label>
      ...
      {loading ? <div className="grid min-h-48 place-items-center"><DotGridLoader label="Loading general ledger" /></div> : !entity ? <p>{t(entities.length ? "Choose a legal entity to view its general ledger." : "No legal entities are available.")}</p> : workspace ? <>
    ...
    <Dialog open={Boolean(journal)} onOpenChange={open => { if (!open && !busy && (!dirty || window.confirm(t("Discard unsaved journal changes?")))) { setJournal(null); setConfirmPost(false); setError("") } }}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl"><DialogHeader><DialogTitle>{journal?.number ? `JN-${journal.number}` : t("New journal")}</DialogTitle><DialogDescription>{t("Use non-control accounts. Save a draft, review the balanced entry, then post. Posted journals cannot be edited. Correct them with a new balancing journal.")}</DialogDescription></DialogHeader>
      ...
      {journal.reversal_of_id && <p className="text-xs text-[var(--md-text)]">{t("Linked reversal of journal")} <span data-i18n-skip>{journal.reversal_of_id}</span>. {t("Nominals and amounts are pinned to the source posting.")}</p>}
      {journal.status === "posted" && canPrepare && !journal.reversal_of_id && <div className="flex flex-wrap items-end gap-3"><label className="grid min-w-[220px] flex-1 gap-1 text-xs">{t("Reason for reversal")}<Input value={reversalReason} maxLength={500} onChange={event => setReversalReason(event.target.value)} /></label><Button variant="outline" disabled={busy || reversalReason.trim().length < 3} onClick={() => void prepareReversal()}>{t("Create linked reversal")}</Button><p className="w-full text-xs text-[var(--md-text)]">{t("The opposite entry is saved as a draft dated the first day of the following month. Review that date before posting.")}</p></div>}
