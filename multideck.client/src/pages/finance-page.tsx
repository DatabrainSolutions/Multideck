...
function DocumentDraftDialog({ type, options, loading, onClose, onCreated }: { type: FinanceDocumentType | null; options: FinanceDraftOptions | null; loading: boolean; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  const availableJobs = (options?.jobs ?? []).filter((job) => (!tenantEntityId || !job.Job_LegalEntityID || job.Job_LegalEntityID === tenantEntityId) && Boolean(ledger === "receivables" ? job.Job_Customer : job.Job_Supplier))
  ...
  const baseCurrencyCode = (tenantEntity?.FinanceDraftCurrencyCode ?? "").toUpperCase()
  ...
  const printProforma = () => {
    ...
    const opened = printFinanceProforma({
      ...
      entityName: tenantEntity?.LegalEntity_Name ?? "",
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t(taxPending || tenantEntity?.FinanceDraftCurrencyStatus !== "approved" ? `${documentLabels[type]} saved as an incomplete draft` : `${documentLabels[type]} draft created`))
      await onCreated()
      onClose()
    } catch (cause) {
...
function CashDraftDialog({ type, options, loading, onClose, onCreated }: { type: FinanceCashType | null; options: FinanceDraftOptions | null; loading: boolean; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  </div>}<DialogFooter><Button type="button" variant="outline" onClick={onClose} disabled={submitting}>{t("Cancel")}</Button><Button type="submit" disabled={loading || submitting || !tenantEntityId || !partyOrgId || !bankAccountId || amount <= 0 || allocated > amount || (needsExchangeRate && Number(exchangeRate) <= 0)}>{submitting ? <LoaderCircle className="animate-spin" /> : <ShieldCheck className="size-4" />}{t("Create draft")}</Button></DialogFooter></form></DialogContent></Dialog>
}
