...
function FinanceSetupPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const retryDelivery = async (id: string) => {
    setRetrying(id)
    try { await processFinanceIntegrationQueue(id); toast.success(t("Accounts system delivery completed.")); await load() } catch (cause) { toast.error(cause instanceof Error ? cause.message : t("Accounts system delivery could not be completed.")); await load() } finally { setRetrying(null) }
  }
