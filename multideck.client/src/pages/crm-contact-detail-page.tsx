      ...
      <Surface padding="none" className="min-w-0 overflow-hidden rounded-[var(--md-radius-xl)]">
        <header className="px-5 py-5 shadow-[var(--md-stroke-bottom)] sm:px-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">{contact.email ? <ContactEmailAction email={contact.email} name={contact.name} className="h-9 px-3 shadow-[var(--md-shadow-line)]"><Mail className="size-4" strokeWidth={1.5} />{t("Email")}</ContactEmailAction> : null}{contact.phone ? <Button asChild variant="outline"><a href={`tel:${contact.phone}`}><Phone className="size-4" strokeWidth={1.5} />{t("Call")}</a></Button> : null}</div>
          </div>
        </header>
          ...
          {contact.emailHistory.length ? contact.emailHistory.map((item, index) => (
            <div key={item.id} className={`grid gap-2 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start sm:px-6 ${index ? "border-t border-[var(--md-line)]" : ""}`}>
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                <ContactEmailAction email={item.email} className="min-w-0 break-all text-[14px] font-medium text-[var(--md-accent)] hover:underline">{item.email}</ContactEmailAction>
