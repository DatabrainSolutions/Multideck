...
const visualizationFoundOn = [
  ...
  { label: "Create report", route: "/reports/new" },
  { label: "Run history", route: "/reports/history" },
...
export const galleryComponents = [
  {
    id: "dexter-voice-controls", name: "Dexter Voice", category: "Communication",
    description: "Quiet microphone controls with speech streamed into the normal conversation.",
    details: "No orb or persistent allowance text. The compact panel supports mute and end; live transcripts use the main chat. When daily voice time runs out, a dismissible notice gently appears above the composer and typing stays available. Reduced motion removes the transition. Personal settings offer named voices with previews. The gallery sample does not open a microphone or spend usage.",
    foundOn: [{ label: "Dexter", route: "/agent-dexter" }, { label: "Dexter settings", route: "/settings?tab=dexter" }],
    componentCode: dexterVoiceSource,
    usageCode: `const voice = useDexterVoice({ conversationId, onRequest: runExistingDexterRequest })\nconst reachedLimit = hasReachedDailyVoiceLimit(voice)\n<DexterVoiceLimitNotice visible={reachedLimit} />\n<DexterPromptComposer voicePanel={voice.phase !== "idle" && (!reachedLimit || voice.active) ? <DexterVoicePanel voice={voice} /> : undefined} {...composerProps} />\n<DexterVoiceSettings />`,
  },
  {
...
<TicketAttachmentList items={message.attachments} />`,
  ...
  },
  {
    ...
    details: "Choose icons by the operator action or product object they represent. Use the shared adapter for the standard 1.5px stroke, current-colour behaviour, refs, accessibility props, and animated state swaps. Use Pen01 for edit and rename actions, matching Calendar event details. The legacy Pencil export resolves to Pen01; do not reintroduce the old PencilIcon glyph.",
    foundOn: [
      { label: "Home", route: "/" },
  ...
  },
  {
    ...
    category: "Forms",
    description: "Scan cargo in a compact table and expand a line to edit its goods, measurements and handling.",
    details: "The parent owns draft saving and version permissions. Summary rows show packages, weight, volume and handling; expanding a row keeps the detail form within the available width. Drafts and saved versions share the same field layout, with saved versions read only. Stable line identifiers and exact totals are preserved. Removing a line requires confirmation and never alters submitted history.",
  ...
  },
  {
    ...
    details: "Use on detail surfaces where operators review more often than they edit. Text, select, switch and grouped variants share keyboard-safe save, error and confirmation behaviour. InlineFieldGroup compact uses right-aligned side labels and visible controls. Short, medium and full widths fit the expected data; long text stays resizable. Existing stacked and read-to-edit layouts are unchanged.",
    foundOn: [
    ...
    componentCode: `${inlineFieldSource}\n\n/* inline-field.css */\n${inlineFieldStyles}`,
    usageCode: `<InlineFieldGroup compact>\n  <InlineField label="Company" value={account.name} onSave={saveName} required />\n  <InlineField label="Tier" width="short" value={account.tier} onSave={saveTier} />\n  <InlineSelectField label="Relationship" width="medium" value={account.type} options={accountTypes} onSave={saveType} />\n  <InlineField label="Summary" kind="textarea" value={account.summary} onSave={saveSummary} />\n</InlineFieldGroup>`,
  },
  {
    id: "wizard-dialog",
    name: "Wizard Dialog",
...
// access the same authenticated Inbox action is completed by Dexter.`,
  ...
  },
  {
    ...
    foundOn: [{ label: "Report viewer", route: "/reports/marlow-may-review" }, { label: "Create report", route: "/reports/new" }, { label: "Components", route: "/components?component=report-data-editor" }],
  ...
  },
  {
    ...
    foundOn: [{ label: "Overview", route: "/" }, { label: "Reports", route: "/reports" }, { label: "Create report", route: "/reports/new" }, { label: "Settings", route: "/settings" }, { label: "Components", route: "/components" }],
...
/>`,
  },
  {
    ...
    foundOn: [{ label: "New expected receipt", route: "/warehouse/purchase-orders/new" }, { label: "Expected receipt detail", route: "/warehouse/purchase-orders/d0c00000-0000-4000-8000-000000000001" }, { label: "Components", route: "/components?component=purchase-order-line-editor" }],
...
/>`,
  ...
  },
  {
    ...
    foundOn: [{ label: "Run history", route: "/reports/history" }, { label: "Create report", route: "/reports/new" }, { label: "Components", route: "/components" }],
  ...
  },
  {
    ...
    foundOn: [{ label: "Run history", route: "/reports/history" }, { label: "Components", route: "/components" }],
  ...
  },
  {
    ...
    foundOn: [{ label: "Overview manual dashboard", route: "/" }, { label: "Run history", route: "/reports/history" }, { label: "Create report", route: "/reports/new" }, { label: "Components", route: "/components" }],
...
/>`,
  },
  {
    ...
    category: "CRM",
    description: "The shared Main Details and address-management surface for editable codes, responsible offices, online-assisted addresses, opening hours, and destination-aware related-party defaults.",
    details: "Use on a company record after its identity and types are established. Main Details owns company controls and related-party defaults; Addresses uses a compact selection table and one form whose values change with the selected row. The editor keeps address fields in a dense left-hand form and purposes with independent defaults in a right-hand list, stacking them on smaller screens.",
  ...
  return (
    <Surface padding="none">
      <MainDetails code={account.accountCode} scope={account.scopeCode} offices={account.officeAssignments} />
...
<Iphone videoSrc={previewVideoUrl} />`,
  ...
  },
  {
    ...
    category: "CRM",
    description: "Four arrangements for a public digital business card, each shown as the card it produces rather than a wireframe.",
