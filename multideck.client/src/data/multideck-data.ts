...
>
  ...
  },
  {
    ...
    name: "Card Miniature",
    category: "CRM",
    description: "A contact card drawn at thumbnail size from its real branding, so every design choice can be picked by looking at the outcome.",
    details: "Use inside any chooser that changes how a public card looks. It resolves the same theme and layout table the public page uses, so a thumbnail can never disagree with what a visitor gets. Pass a modified copy of the branding to show one option.",
    foundOn: [{ label: "Contact Card design", route: "/crm/contact-cards" }, { label: "Components", route: "/components?component=card-miniature" }],
    componentCode: `export function CardMiniature({ branding }) {\n  const theme = resolveCardTheme(branding)\n  const spec = resolveCardLayout(branding.layout)\n\n  return (\n    <span style={{ backgroundColor: theme.pageBg }}>\n      <Header style={branding.headerStyle} accent={theme.accent} />\n      <Identity placement={spec.mark.placement} size={spec.mark.size} />\n      <Fields variant={spec.field.variant} action={theme.actionBg} />\n    </span>\n  )\n}`,
    usageCode: `<CardMiniature branding={{ ...card.branding, layout: "editorial" }} />`,
  },
  {
    id: "contact-card-style-picker",
    name: "Contact Card Style Picker",
    category: "CRM",
    description: "Six whole-card looks, each drawn in the card's own accent, as the starting point for designing a public card.",
    details: "Use at the top of a card design surface. A style moves the arrangement, header, theme, corners and code pattern together and deliberately leaves the accent and logo alone, so a one-click look can never overwrite someone's brand.",
  ...
  },
  {
    ...
    details: "Use in Contact Card design settings. The thumbnails carry the card's live colours, header and corners, so the only visible difference between the four is the thing being chosen. Supports keyboard selection and leaves colour and QR styling independent.",
  ...
  },
  {
    ...
    name: "Contact Card Header Picker",
    category: "CRM",
    description: "The four ways a public card can open: nothing, an accent rule, a logo band, or a colour field for the mark to sit over.",
    details: "Use beside the layout picker. Each option renders the real header against the card's live colours, so the difference between a band and a cover is visible instead of described.",
    foundOn: [{ label: "Contact Card design", route: "/crm/contact-cards" }, { label: "Components", route: "/components?component=contact-card-header-picker" }],
    componentCode: `export function ContactCardHeaderPicker({ value, onChange, branding }) {\n  return (\n    <div role="radiogroup" aria-label="Header">\n      {HEADER_CHOICES.map((choice) => (\n        <OptionTile\n          selected={value === choice.id}\n          label={choice.label}\n          detail={choice.detail}\n          onSelect={() => onChange(choice.id)}\n        >\n          <CardMiniature branding={{ ...branding, headerStyle: choice.id }} />\n        </OptionTile>\n      ))}\n    </div>\n  )\n}`,
    usageCode: `<ContactCardHeaderPicker\n  branding={card.branding}\n  value={card.branding.headerStyle}\n  onChange={(headerStyle) => updateBranding(card.id, { headerStyle })}\n/>`,
  },
  {
    ...
    foundOn: [{ label: "Contact Card design", route: "/crm/contact-cards" }, { label: "Components", route: "/components?component=contact-card-qr-style-picker" }],
