...
export const dashboardSnapshots: Record<DashboardRange, {
  ...
  today: {
    ...
    briefItems: [
      ...
      { label: "Approve Yong Hua chase", value: "Approve", detail: "Artie drafted the licence request for MD-22455. Check wording, then send when you are ready.", source: "Customs hold", tone: "red" },
  ...
  week: {
    ...
    briefItems: [
      ...
      { label: "Review customs pattern", value: "Wed", detail: "Three licence holds share the same broker handoff gap. Prepare a rule change for Artie.", source: "Customs", tone: "red" },
      { label: "Schedule customer updates", value: "Fri", detail: "Bundle the non-urgent ETA and milestone updates into account-level summaries.", source: "Artie brief", tone: "teal" },
    ],
    kpis: [
  ...
  month: {
    ...
    briefItems: [
      ...
      { label: "Reduce detention risk", value: "9", detail: "Most open risks are customs holds or containers nearing free-time expiry.", source: "Artie brief", tone: "red" },
    ],
    kpis: [
  ...
  quarter: {
    ...
    briefItems: [
      ...
      { label: "Fix repeated customs patterns", value: "7", detail: "Licence, HS-code, and detention issues repeat by lane. These should become watchers, not surprises.", source: "Artie brief", tone: "red" },
    ],
    kpis: [
  ...
  custom: {
    ...
    briefItems: [
      ...
      { label: "Prepare customer recap", value: "Draft", detail: "Summarise movement, risk, and next steps for the selected operating period.", source: "Artie brief", tone: "teal" },
    ],
    kpis: [
...
export const operatorJobs = [
  ...
  },
  {
    ...
    detail: "Artie drafted the update from the latest ETA model.",
    due: "13:40",
    status: "Draft ready",
    tone: "teal" as StatusTone,
  },
...
export const crmActivities = [
  ...
  { time: "Mon 14:20", account: "Bauhaus Importe GmbH", title: "Renewal risk note added", source: "Artie · Account pulse", detail: "Rotterdam delay communication may affect renewal confidence if not handled today.", tone: "amber" as StatusTone },
]
...
export const galleryComponents = [
  ...
  },
  {
    ...
    name: "Artie Prompt Composer",
    category: "Agent Artie",
    description: "The central command box for Agent Artie: prompt text, attached context, specialist choice, and send action.",
    details: "Use on the Agent Artie landing and conversation footer. It should feel like the operator can hand over a whole job from one calm control.",
    foundOn: [{ label: "Agent Artie", route: "/agent-artie" }, { label: "Components", route: "/components" }],
    componentCode: `export function ArtiePromptComposer({ value, selectedSpecialist, attachments, onChange, onOpenAttachments, onOpenSpecialists, onSend }) {\n  return (\n    <div className="rounded-[22px] bg-[var(--md-surface)] p-1.5 shadow-[0_0_0_1px_rgba(14,125,116,0.36)]">\n      <textarea value={value} onChange={(event) => onChange(event.target.value)} />\n      {attachments.map((attachment) => <ContextChip key={attachment.id} attachment={attachment} />)}\n      <button onClick={onOpenAttachments}>Attach</button>\n      <button onClick={onOpenSpecialists}>{selectedSpecialist.name}</button>\n      <button onClick={onSend}>Send</button>\n    </div>\n  )\n}`,
    usageCode: `<ArtiePromptComposer\n  value={prompt}\n  selectedSpecialist={selectedSpecialist}\n  attachments={attachedItems}\n  onChange={setPrompt}\n  onOpenAttachments={() => setShowAttachments(true)}\n  onOpenSpecialists={() => setShowSpecialists(true)}\n  onSend={startConversation}\n/>`,
  },
  {
    id: "artie-specialist-picker",
    name: "Artie Specialist Picker",
    category: "Agent Artie",
  ...
  },
  {
    ...
    foundOn: [{ label: "Agent Artie", route: "/agent-artie" }, { label: "Components", route: "/components" }],
    componentCode: `export function ArtieSpecialistMenu({ specialists, selectedId, onSelect }) {\n  return (\n    <Surface className="w-[300px]">\n      {specialists.map((specialist) => (\n        <button key={specialist.id} onClick={() => onSelect(specialist.id)}>\n          <specialist.icon />\n          <span>{specialist.name}</span>\n          <span>{specialist.description}</span>\n        </button>\n      ))}\n    </Surface>\n  )\n}`,
    usageCode: `<ArtieSpecialistMenu\n  specialists={defaultArtieSpecialists}\n  selectedId={selectedSpecialistId}\n  onSelect={setSelectedSpecialistId}\n/>`,
  },
  {
    id: "artie-attachment-palette",
    name: "Artie Attachment Palette",
    category: "Agent Artie",
    ...
    foundOn: [{ label: "Agent Artie", route: "/agent-artie" }, { label: "Components", route: "/components" }],
    componentCode: `export function ArtieAttachmentPalette({ query, items, selectedIds, onQueryChange, onToggle }) {\n  const filtered = items.filter((item) => item.title.toLowerCase().includes(query.toLowerCase()))\n\n  return (\n    <Surface>\n      <input value={query} onChange={(event) => onQueryChange(event.target.value)} />\n      {filtered.map((item) => (\n        <button key={item.id} onClick={() => onToggle(item.id)}>\n          <span>{item.title}</span>\n          <span>{item.meta}</span>\n          <span>{selectedIds.has(item.id) ? "Attached" : "Attach"}</span>\n        </button>\n      ))}\n    </Surface>\n  )\n}`,
    usageCode: `<ArtieAttachmentPalette\n  query={attachmentQuery}\n  items={defaultArtieAttachments}\n  selectedIds={selectedAttachmentIds}\n  onQueryChange={setAttachmentQuery}\n  onToggle={toggleAttachment}\n/>`,
  },
  {
    id: "artie-history-list",
    name: "Artie History List",
    category: "Agent Artie",
    description: "The conversation rail for jumping between recent Artie jobs without leaving the AI workspace.",
    details: "Use only inside the Agent Artie workspace. It should behave like a focused work history, not a full inbox.",
    foundOn: [{ label: "Agent Artie", route: "/agent-artie" }, { label: "Components", route: "/components" }],
    componentCode: `export function ArtieHistoryList({ items, activeId, onSelect, onNew }) {\n  return (\n    <aside>\n      <button onClick={onNew}>New</button>\n      {items.map((item) => (\n        <button key={item.id} aria-current={activeId === item.id ? "page" : undefined} onClick={() => onSelect(item.id)}>\n          <span>{item.title}</span>\n          <span>{item.summary}</span>\n          <span>{item.time}</span>\n        </button>\n      ))}\n    </aside>\n  )\n}`,
    usageCode: `<ArtieHistoryList\n  items={historyItems}\n  activeId={activeHistoryId}\n  onSelect={setActiveHistoryId}\n  onNew={startNewConversation}\n/>`,
  },
  {
    id: "artie-monitor-card",
    name: "Artie Monitor Card",
    category: "Agent Artie",
    description: "A compact background-monitor card for showing what Artie is watching on the operator's behalf.",
    details: "Use in the right rail of Agent Artie. Each card needs a trigger, what Artie is watching, and when it will next matter.",
    foundOn: [{ label: "Agent Artie", route: "/agent-artie" }, { label: "Components", route: "/components" }],
    componentCode: `export function ArtieMonitorCard({ monitor }) {\n  return (\n    <Surface>\n      <span style={{ background: toneToVar(monitor.tone) }} />\n      <h3>{monitor.title}</h3>\n      <p>{monitor.body}</p>\n      <footer>{monitor.meta} - {monitor.detail}</footer>\n    </Surface>\n  )\n}`,
    usageCode: `<ArtieMonitorStack monitors={monitors} onAsk={openWatcherComposer} />`,
  },
  {
    id: "artie-monitor-detail",
    name: "Artie Monitor Detail",
    category: "Agent Artie",
    ...
    foundOn: [{ label: "Agent Artie", route: "/agent-artie" }, { label: "Components", route: "/components" }],
    componentCode: `export function ArtieMonitorDetailSheet({ monitor, onClose }) {\n  return (\n    <aside className="fixed inset-y-0 right-0">\n      <button onClick={onClose}>Close</button>\n      <h2>{monitor.title}</h2>\n      <StatusPill tone="green">Active</StatusPill>\n      <section>Watching condition and linked shipment/customer context</section>\n      <section>Trend chart and activity timeline</section>\n      <footer>\n        <button>Pause</button>\n        <button>Edit conditions</button>\n        <button>Delete</button>\n      </footer>\n    </aside>\n  )\n}`,
    usageCode: `<AnimatePresence>\n  {selectedMonitor ? (\n    <ArtieMonitorDetailSheet\n      monitor={selectedMonitor}\n      onClose={() => setSelectedMonitor(null)}\n    />\n  ) : null}\n</AnimatePresence>`,
  },
  {
    id: "artie-response-blocks",
    name: "Artie Response Blocks",
    category: "Agent Artie",
    description: "Structured answer blocks for customer snapshots, action checklists, and shipment risk tables inside Artie conversations.",
    details: "Use when Artie returns something the operator should scan or act on. The answer should feel operational, not like a long chat transcript.",
    foundOn: [{ label: "Agent Artie", route: "/agent-artie" }, { label: "Components", route: "/components" }],
    componentCode: `<ArtieCustomerSnapshot />\n<ArtieChecklistCard items={steps} />\n<ArtieRiskTable />`,
...
export const galleryCategories = ["All", "Design System", "Foundation", "Navigation", "Data", "Visualizations", "Feedback", "Operations", "CRM", "Agent Artie"]
...
export const galleryIcons = {
  ...
  "artie-prompt-composer": Sparkles,
  "artie-specialist-picker": PackageCheck,
  "artie-specialist-menu": PackageCheck,
  "artie-attachment-palette": Search,
  "artie-history-list": ListOrdered,
  "artie-monitor-card": Bell,
  "artie-monitor-detail": Bell,
  "artie-response-blocks": BarChart3,
