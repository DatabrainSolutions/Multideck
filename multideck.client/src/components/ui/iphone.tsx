...
// The frame scales, so the safe content line is proportional to screen width.
// It clears the 47.75-unit notch depth and leaves a quiet gap underneath it.
export const IPHONE_CONTENT_SAFE_TOP = "16%"
...
export function Iphone({
  ...
  const screenMaskId = `iphone-screen-${id}`
  const roundedCornersId = `iphone-corners-${id}`
  ...
  return (
    ...
    >
      ...
      {children ? (
        <div
          className="absolute z-0 overflow-x-hidden overflow-y-auto"
          style={{
            left: `${LEFT_PCT}%`,
            top: `${TOP_PCT}%`,
            width: `${WIDTH_PCT}%`,
