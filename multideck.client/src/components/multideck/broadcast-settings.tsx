...
function BroadcastHistory({ state, busy, loadingMore, openWizard, onLoadMore, t }: { state: BroadcastState | null; busy: BroadcastSettingsBusy; loadingMore: boolean; openWizard: (trigger?: HTMLButtonElement) => void; onLoadMore: () => void; t: (text: string) => string }) {
  ...
  if (!history.length) return <div className="px-5 py-12 text-center"><p className="text-[13px] font-medium text-[var(--md-ink)]">{busy === "load" ? t("Loading broadcast history…") : t("No broadcasts yet")}</p>{busy !== "load" ? <Button type="button" variant="outline" className="mt-4 h-9 rounded-[var(--md-radius-md)] active:scale-[0.96] motion-reduce:active:scale-100" onClick={(event) => openWizard(event.currentTarget)}><Plus className="size-4" />{t("New broadcast")}</Button> : null}</div>
...
export function BroadcastSettings() {
  ...
  return (
    <>
      <SettingsPageHeader title={t("Broadcast")} description={t("Email workspace users. Review recipients and the message before sending.")} descriptionPlacement="under-title" icon={Megaphone} actions={<div className="flex flex-wrap gap-2"><Button type="button" variant="ghost" className="h-9 rounded-[var(--md-radius-md)] px-3 text-[12px] active:scale-[0.96] motion-reduce:active:scale-100" onClick={() => void load()} disabled={busy !== null}><RefreshCw className={cn("size-3.5", busy === "load" && "animate-spin motion-reduce:animate-none")} />{t("Refresh")}</Button><Button type="button" className="h-9 rounded-[var(--md-radius-md)] bg-[var(--md-accent)] px-4 text-[var(--md-accent-ink)] hover:bg-[var(--md-accent-hover)] active:scale-[0.96] motion-reduce:active:scale-100" onClick={(event) => openWizard(event.currentTarget)}><Plus className="size-4" strokeWidth={1.5} />{t("New broadcast")}</Button></div>} />
...
function ComposeStep({ subject, message, direction, busy, onSubject, onMessage, onDirection, onAI, t }: { subject: string; message: string; direction: string; busy: boolean; onSubject: (value: string) => void; onMessage: (value: string) => void; onDirection: (value: string) => void; onAI: () => void; t: (text: string) => string }) {
  ...
  return <>
    ...
    <div className="grid gap-2 text-[13px] font-medium text-[var(--md-ink)]"><span>{t("Message")}</span><BroadcastMessageEditor value={message} onChange={onMessage} t={t} /></div>
