...
export function CargoHandlingEditor({ value, line, editable, onChange }: {
  ...
  return <div className="grid gap-2 text-[12px]">
    ...
    <Dialog open={editing !== null} onOpenChange={open => { if (!open) setEditing(null) }}>
      <DialogContent onCloseAutoFocus={event => { event.preventDefault(); if (trigger.current?.isConnected) trigger.current.focus() }}><DialogHeader><DialogTitle>{editing ? t(handlingLabels[editing]) : ''}</DialogTitle><DialogDescription>{t('Record supplied requirements for this cargo line. TBC allows Quote issue but must be resolved before Booking readiness and completion. This is not carrier or safety approval.')}</DialogDescription></DialogHeader>
        <div className="grid max-h-[55vh] gap-3 overflow-y-auto sm:grid-cols-2">
          {editing ? handlingFields[editing].map(([key, label]) => <label key={key} htmlFor={`${id}-${key}`} className="grid gap-1 text-[12px]">{t(label)}<Input id={`${id}-${key}`} value={draft.details[key] ?? ''} onChange={event => setDraft({ ...draft, details: { ...draft.details, [key]: event.target.value } })} /></label>) : null}
        </div>
        <label className="flex min-h-8 items-center gap-2 text-[12px]"><Checkbox checked={draft.tbc} onCheckedChange={checked => setDraft({ ...draft, tbc: checked === true })} />{t('Details TBC')}</label>
        <DialogFooter><Button type="button" variant="ghost" onClick={() => { if (editing && editable) { const next = { ...handling }; delete next[editing]; onChange(JSON.stringify(next)) }; setEditing(null) }}>{t('Remove selection')}</Button><Button type="button" variant="outline" onClick={() => setEditing(null)}>{t('Cancel')}</Button><Button type="button" onClick={() => { if (editing && editable) onChange(JSON.stringify({ ...handling, [editing]: draft })); setEditing(null) }}>{t('Apply details')}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
}
