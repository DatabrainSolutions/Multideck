// Adapted from React Bits "TearTicket" (https://reactbits.dev/micro/tear-ticket,
// TS + Tailwind registry item). Geometry, perforation, image band and parallax
// art are the React Bits source. Multideck changes: design-token colours, no
// rotation or hover tilt by default, an adjustable image band, no grey "used"
// treatment, and `interactive={false}` — a display-only mode with no drag,
// keyboard tear, fibres, onTear or dimming, used where attendance is an
// explicit button instead of a gesture.
import React, {
  useEffect,
  useLayoutEffect,
  useMemo,
...
export interface TearTicketProps {
  ...
  ariaLabel?: string;
  imageSpan?: number;
  /** False renders the ticket only: the stub cannot be torn and nothing is dimmed. */
  interactive?: boolean;
  className?: string;
}
...
const TearTicket: React.FC<TearTicketProps> = ({
  ...
  const used = interactive && (controlled ? torn : inner);
  const inert = disabled || !interactive;
  ...
  const onStubDown = (e: PointerEvent<HTMLDivElement>) => {
    const s = sim.current;
    if (inert || used || e.button !== 0 || s.id !== null || s.phase === 'drop') return;
    try {
  ...
  const onStubKey = (e: KeyboardEvent<HTMLDivElement>) => {
    if (inert || used || (e.key !== 'Enter' && e.key !== ' ')) return;
    e.preventDefault();
    if (!e.repeat) tearNow();
  };

  useEffect(() => {
    if (!tilt || reduce || inert) return undefined;
  ...
  return (
    <div
      ...
      data-disabled={disabled && interactive ? '' : undefined}
      style={
        {
          '--tt-w': `${width}px`,
          '--tt-h': `${height}px`,
        ...
        <motion.div className="absolute inset-0" style={{ transform: plane }}>
          <div ref={bodyRef} className="pointer-events-none absolute inset-0">
            ...
            >
              ...
              <motion.div
                className="absolute inset-y-0 left-0 [width:calc(var(--tt-w)_-_var(--tt-stub))] group-data-[orientation=vertical]:bottom-auto group-data-[orientation=vertical]:w-full group-data-[orientation=vertical]:[height:calc(var(--tt-h)_-_var(--tt-stub))]"
          ...
          <svg
            ...
            aria-hidden="true"
          >
            {interactive && geo.bridges.map((b, i) => (
              <g key={i}>
                <path
                  ref={el => {
          ...
          >
            ...
            <div
              className="pointer-events-auto absolute inset-0 [background:var(--tt-stub-bg)] group-data-[interactive]:cursor-grab group-data-[interactive]:touch-none group-data-[grabbing]:cursor-grabbing group-data-[disabled]:cursor-default"
