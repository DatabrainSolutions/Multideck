...
export function PublicBrandIdentity({ brand }: { brand?: PublicBranding | null }) {
  ...
  return <div className="flex min-h-11 min-w-0 max-w-[200px] items-center">
    {logo ? <img key={logo} src={logo} alt={brand?.displayName || "Company logo"} onError={() => setFailedUrl(logo)} className="block max-h-11 min-w-0 max-w-full object-contain object-left" />
