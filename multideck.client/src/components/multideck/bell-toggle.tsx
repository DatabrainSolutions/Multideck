// Adapted from the React Bits BellToggle supplied for this integration.
...
type BellToggleProps = {
  offLabel?: string; onLabel?: string; icon?: ReactNode; label?: string;
  color?: string; background?: string; onColor?: string; onBackground?: string;
  size?: 'sm' | 'md' | 'lg'; radius?: number; ringAmplitude?: number;
  ringPasses?: number; ringDecay?: number; ringDuration?: number; ringPivot?: number;
  crossfadeMs?: number; revealBounce?: number; count?: number; badge?: boolean;
  badgeColor?: string; badgeTextColor?: string; waves?: boolean; clapper?: boolean;
  pressed?: boolean; defaultPressed?: boolean; onChange?: (pressed: boolean) => void;
...
export function BellToggle({
  ...
  onColor = 'var(--md-accent-ink)',
  onBackground = 'var(--md-accent)',
  size = 'md',
  radius = 22,
  ringAmplitude = 17,
  ringPasses = 5,
  ...
  const swing = (amplitude: number, passes: number, duration: number) => {
    ...
    if (!el) return;
    const angle = liveAngle(el);
    el.getAnimations().forEach(a => a.cancel());
    el.animate(ringKeyframes(angle, amplitude, passes, ringDecay), { duration, easing: 'linear' });
