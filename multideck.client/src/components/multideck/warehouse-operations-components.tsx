          ...
          <div className="grid gap-3">
            ...
            {form.lines.filter((line) => line.key === activeLineKey).map((line) => {
              ...
              return <div key={line.key} className="grid gap-3 rounded-[var(--md-radius-xl)] bg-white/36 p-4 shadow-[var(--md-shadow-line)] md:grid-cols-12">
                ...
                })}</SelectContent></Select></WarehouseFormField> : <div className="md:col-span-4 rounded-[var(--md-radius-lg)] bg-[rgba(14,125,116,0.07)] px-3 py-2 text-[12px] leading-5 text-[var(--md-text)]">{t("Warehouse staff will assign the storage or picking location.")}</div>}
    ...
    <Tabs value={section} onValueChange={setSection} className="h-[512px] gap-0">
      ...
      <TabsContent value="documents" className="grid min-h-0 content-start gap-4 overflow-y-auto px-6 py-5">
        <div className="flex items-start justify-between gap-4">
          ...
          {canUpload ? <label className="inline-flex h-9 cursor-pointer items-center gap-2 rounded-[var(--md-radius-lg)] bg-[var(--md-accent)] px-3 text-[12px] font-medium text-white shadow-[0_10px_22px_rgba(14,125,116,0.14)]">
