...
export function BookingCustomerPanel({ customerId, customerName, contactId, onAssignCustomer, loadCustomer = getCustomer, reducedMotion = false }: {
  ...
  const hasPreferences = Boolean(preferred || restrictions.length || engagement?.doNotOverContact || engagement?.notes?.trim())
  function contactAction(kind: "email" | "phone", value: string | null | undefined, name: string) {
    ...
    return <a className={actionClass} href={href} aria-label={`${t(kind === "email" ? "Email" : "Call")} ${name}`}><Icon aria-hidden="true" className="size-3.5 shrink-0" /><span className="break-all" data-i18n-skip>{value}</span></a>
  }
  return <CompactSectionShell title="Customer" className="h-full" contentClassName="min-h-52" action={customer ? <div className="flex items-center gap-3">{selectedContact ? <button id={`${panelId}-back`} type="button" className={actionClass} onClick={backToCustomer} aria-label={t("Back to customer")}><ArrowLeft className="size-3.5" aria-hidden="true" />{t("Back")}</button> : null}<a className={actionClass} href={`/crm/accounts/${encodeURIComponent(customer.id)}`} target="_blank" rel="noopener noreferrer" aria-label={t("Open customer account in a new tab")}><span>{t("Account")}</span><ArrowUpRight aria-hidden="true" className="size-3.5" /></a></div> : undefined}>
    {!customerId ? <div className="flex min-h-48 flex-col items-center justify-center gap-2 text-center">
    ...
    </div> : active?.error ? <div className="flex min-h-48 flex-col items-start justify-center gap-2"><p role="alert" className="text-[12px] text-[var(--md-subtle)]">{t(active.error)}</p><Button variant="outline" size="sm" onClick={() => setRetry(value => value + 1)}>{t("Try again")}</Button></div> : !customer ? <div className="min-h-48 py-3" role="status" aria-busy="true"><p className="text-[12px] text-[var(--md-subtle)]">{t("Loading customer…")}</p></div> : <motion.div key={`${customer.id}-${selectedContact?.id ?? "customer"}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={prefersReducedMotion || reducedMotion ? { duration: 0 } : mdMotion.micro} className="grid gap-4">
      <div className="flex min-w-0 items-start gap-2.5"><span aria-hidden="true" className="grid size-9 shrink-0 place-items-center rounded-[var(--md-radius-lg)] bg-[var(--md-accent-a10)] text-[var(--md-accent)]"><Building2 className="size-4" /></span><div className="min-w-0"><p className="break-words text-[14px] font-medium" data-i18n-skip>{customer.name}</p>{customer.accountCode ? <p className="mt-0.5 text-[11px] text-[var(--md-subtle)]" data-i18n-skip>{customer.accountCode}</p> : null}<div className="flex flex-wrap gap-x-4">{contactAction("email", customer.address?.mainEmail, customer.name)}{contactAction("phone", customer.address?.mainPhone, customer.name)}</div>{!customer.address?.mainEmail ? <p className="mt-1 text-[12px] text-[var(--md-subtle)]">{t("No account email saved")}</p> : null}</div></div>
      {selectedContact ? <div className="grid gap-2">
        <h4 className="text-[14px] font-medium" data-i18n-skip>{selectedContact.name}</h4>
        {selectedContact.jobTitle || selectedContact.role ? <p className="text-[12px] text-[var(--md-subtle)]" data-i18n-skip>{selectedContact.jobTitle || selectedContact.role}</p> : null}
        <div className="flex flex-wrap gap-x-4">{contactAction("email", selectedContact.email, selectedContact.name)}{contactAction("phone", selectedContact.phone, selectedContact.name)}</div>
        {!selectedContact.email && !selectedContact.phone ? <p className="text-[12px] text-[var(--md-subtle)]">{t("No contact details saved")}</p> : null}
        {selectedContact.preferredChannel ? <p className="text-[12px]">{t("Preferred")}: <span data-i18n-skip>{channelLabels[selectedContact.preferredChannel] ? t(channelLabels[selectedContact.preferredChannel]) : selectedContact.preferredChannel}</span></p> : null}
        <a className={actionClass} href={`/crm/contacts/${encodeURIComponent(selectedContact.id)}`} target="_blank" rel="noopener noreferrer">{t("Contact profile")}<ArrowUpRight aria-hidden="true" className="size-3.5" /></a>
      </div> : null}
      <div className="grid gap-1.5"><h4 className="text-[11px] font-medium text-[var(--md-subtle)]">{t("Preferences")}</h4>
        ...
        {preferred ? <p className="text-[12px]">{t("Preferred")}: <span data-i18n-skip>{channelLabels[preferred] ? t(channelLabels[preferred]) : preferred}</span></p> : null}
