...
export type StandaloneExportDraft = {
  ...
  representativeOrganisationId?: string
  representativeAddressId?: string
  buyer: string
  consignee: string
  ...
  declarantOrganisationId?: string
  declarantAddressId?: string
  ...
  goodsAddressType?: string
  goodsLocationCountry?: string
  goodsLocationAdditionalIdentifier?: string
  additionalContainerIds?: string[]
...
export function createStandaloneDeclarationDraft(direction: DeclarationDirection): StandaloneExportDraft {
  return {
    ...
    declarationCategory: direction === "import" ? "H1" : "",
    declarationType: "",
    badgeId: "",
    ucn: "",
    traderReference: "",
...
export function validateStandaloneExportDraft(draft: StandaloneExportDraft): DeclarationIssue[] {
  ...
  if (draft.direction === "import") {
    for (const [index, issue] of importDefermentIssues(draft).entries()) issues.push({ id: `deferment-${index}`, scope: issue.itemIndex == null ? "general" : "item", field: issue.field, ...(issue.itemIndex == null ? {} : { itemId: draft.items[issue.itemIndex].id }), message: issue.message })
    if (draft.importerEori && !/^[A-Z]{2}[A-Z0-9]{3,15}$/.test(draft.importerEori.trim())) issues.push({ id: "general-importerEori", scope: "general", field: "importerEori", message: "Check the importer EORI number in the company Customs tab." })
    for (const holder of draft.additionalAuthorisationHolders ?? []) {
      if (!/^[A-Z0-9]{1,4}$/.test(holder.category) || !/^[A-Z]{2}[A-Z0-9]{3,15}$/.test(holder.identifier)) issues.push({ id: `general-authorisation-${holder.id}`, scope: "general", field: "additionalAuthorisationHolders", message: "Complete every additional authorisation category and holder EORI." })
    }
    for (const field of ["primaryDefermentAccount", "secondaryDefermentAccount"] as const) {
      if (draft[field] && !/^\d{7}$/.test(draft[field].trim())) issues.push({ id: `general-${field}`, scope: "general", field, message: "Use the seven-digit deferment account number." })
    }
    requireGeneral("badgeId", "Select a badge code.")
    const error = ducrFormatError(draft.ducr)
    if (error) issues.push({ id: "general-ducr", scope: "general", field: "ducr", message: error })
  }
  ...
  if (draft.direction === "import") for (const issue of additionalCustomsPartyIssues(draft)) issues.push({ id: `general-${issue.field}`, scope: "general", ...issue })
  if (draft.exporterEori && !/^[A-Z]{2}[A-Z0-9]{3,15}$/.test(draft.exporterEori.trim())) {
    issues.push({ id: "general-exporter-eori", scope: "general", field: "exporterEori", message: "Enter a valid exporter EORI: two country letters followed by up to 15 letters or numbers." })
  }
  ...
  if (draft.declarantEori !== undefined && !/^[A-Z0-9]{3,17}$/.test(draft.declarantEori)) issues.push({ id: "general-declarant-eori", scope: "general", field: "declarantEori", message: "Add the declarant’s registered EORI or customs identifier." })
  ...
  if (draft.direction === "import") {
    requireGeneral("inlandMode", "Select the inland transport mode.")
    requireGeneral("borderNationality", "Select the border transport nationality.")
  }
  ...
  if ((draft.gvmsCode || draft.gvmsValue) && (draft.gvmsCode !== "RRS01" || !draft.gvmsValue.trim())) {
    issues.push({ id: "general-gvms", scope: "general", field: draft.gvmsCode !== "RRS01" ? "gvmsCode" : "gvmsValue", message: "Select RRS01 and enter the haulier EORI or name for GVMS." })
  }
