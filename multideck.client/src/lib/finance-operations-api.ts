...
export type FinanceWorklist = { entityId: string; ledger: "receivables" | "payables"; generatedAt: string; asOf: string; items: AgedFinanceItem[] }
...
export const approveSupplierInvoiceMatch = (documentId: string, purchaseOrderId: string, reason: string, proposalEvidence?: unknown) => post<{ FINPOMatch_ID: string }>("/matches", { documentId, purchaseOrderId, reason, proposalEvidence })
...
export const preparePaymentRun = (input: { bankAccountId: string; documentIds: string[]; paymentDate: string; exchangeRate: number; reason: string }) => post<{ runId: string; status: string }>("/payment-runs", input)
