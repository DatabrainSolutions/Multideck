...
{
    [Fact]
    public async Task MapsRequesterAndKeepsSecretOutOfPayload()
    {
        HttpRequestMessage? captured = null;
        var service = CreateService(new StubHandler(async request =>
        {
            captured = await CloneRequest(request);
            return Json(HttpStatusCode.Created, Success());
        }));

        var response = await service.CreateAsync(Request("support-form-stable-key"), Requester(), CancellationToken.None);

        Assert.Equal("TK-2048", response.Ticket.TicketNumber);
        Assert.NotNull(captured);
        Assert.Equal("server-only-test-secret", captured.Headers.GetValues("X-Databrain-Webhook-Secret").Single());
    ...
    [Fact]
    public async Task ReturnsOriginalTicketForDuplicateReplay()
    {
        var service = CreateService(new StubHandler(_ => Task.FromResult(Json(
            HttpStatusCode.OK,
    ...
    [Fact]
    public async Task MapsChangedContentWithSameKeyToConflict()
    {
        var service = CreateService(new StubHandler(_ => Task.FromResult(Json(
            HttpStatusCode.Conflict,
            """{"error":"idempotency_key_reused_with_different_payload"}"""))));

        var error = await Assert.ThrowsAsync<SupportTicketException>(() =>
            service.CreateAsync(Request("support-form-stable-key"), Requester(), CancellationToken.None));
    ...
    [Fact]
    public async Task TimesOutWithoutInventingSuccess()
    {
        var service = CreateService(new StubHandler(async request =>
        {
        ...
        var error = await Assert.ThrowsAsync<SupportTicketException>(() =>
            service.CreateAsync(Request("support-form-timeout-key"), Requester(), CancellationToken.None));
    ...
    [Fact]
    public async Task RejectsResponseWithoutConfirmedTicketNumber()
    {
        var service = CreateService(new StubHandler(_ => Task.FromResult(Json(
        ...
        var error = await Assert.ThrowsAsync<SupportTicketException>(() =>
            service.CreateAsync(Request("support-form-invalid-response"), Requester(), CancellationToken.None));
    ...
    private static SupportTicketService CreateService(HttpMessageHandler handler, int timeoutSeconds = 10) =>
        new(
            ...
            Options.Create(new SupportTicketOptions
            {
    ...
    private static CreateSupportTicketRequest Request(string key) =>
        new(key, "Security concern", "Urgent", "Cannot complete the booking",
    ...
    private static TeamUserDto Requester() =>
        new(
            Guid.NewGuid(), Guid.NewGuid(), "Alex Operator", "Alex", "Operator", "alex@example.com",
            new TeamCompanyDto(Guid.NewGuid(), "Example Logistics"), [], [], "Active");

    private static string Success(bool duplicate = false) => $$"""
        {
          "ticket": {
...
internal static class HttpRequestMessageCancellation
{
    ...
    public static CancellationToken GetCancellationToken(this HttpRequestMessage request) =>
        request.Options.TryGetValue(Key, out var cancellationToken) ? cancellationToken : CancellationToken.None;
}
