...
import { DataCard, EmptyState, ErrorState, LoadingState, MetricRow, ScanField, SuccessState, WarehouseButton, WarehouseScreen } from "@/components/WarehouseUI"
import { colors, spacing, type } from "@/theme/tokens"
import type { WarehouseHandlingUnit, WarehouseInventoryBalance, WarehouseInventoryException, WarehouseItem, WarehouseMobileApi } from "@/warehouse/api"
...
export function StockEnquiryScreen({ api, onBack }: { api: WarehouseMobileApi; onBack: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try { setRows(await api.listInventory({ search: query.trim() })) } catch (loadError) { setError(message(loadError)) } finally { setLoading(false) }
  }, [api, query])
  ...
  return <WarehouseScreen title={wt("stockEnquiry")} subtitle={wt("stockEnquiryDetail")} onBack={onBack}>
    ...
    {loading ? <LoadingState /> : error ? <ErrorState message={error} /> : rows.length ? rows.map((row) => <DataCard key={row.id} title={row.sku} meta={`${row.itemDescription} · ${row.locationCode || "—"}`} status={row.inventoryStatusCode}>
...
export function StockItemsScreen({ api, onBack }: { api: WarehouseMobileApi; onBack: () => void }) {
  const [query, setQuery] = useState("")
  const [rows, setRows] = useState<WarehouseItem[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const load = useCallback(async () => { setLoading(true); setError(null); try { setRows(await api.listItems({ search: query.trim() })) } catch (loadError) { setError(message(loadError)) } finally { setLoading(false) } }, [api, query])
  useEffect(() => { void load() }, []) // eslint-disable-line react-hooks/exhaustive-deps
  return <WarehouseScreen title={wt("stockItems")} subtitle={wt("stockItemsDetail")} onBack={onBack}>
    <ScanField value={query} onChangeText={setQuery} onSubmit={() => void load()} autoFocus />
    {loading ? <LoadingState /> : error ? <ErrorState message={error} /> : rows.length ? rows.map((row) => <DataCard key={row.id} title={row.sku} meta={row.description} status={row.baseUomCode}>
...
export function PalletsScreen({ api, onBack }: { api: WarehouseMobileApi; onBack: () => void }) {
  const [query, setQuery] = useState("")
  const [rows, setRows] = useState<WarehouseHandlingUnit[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const load = useCallback(async () => { setLoading(true); setError(null); try { setRows(await api.listHandlingUnits({ search: query.trim() })) } catch (loadError) { setError(message(loadError)) } finally { setLoading(false) } }, [api, query])
  useEffect(() => { void load() }, []) // eslint-disable-line react-hooks/exhaustive-deps
  return <WarehouseScreen title={wt("pallets")} subtitle={wt("palletsDetail")} onBack={onBack}>
    <ScanField value={query} onChangeText={setQuery} onSubmit={() => void load()} autoFocus />
    {loading ? <LoadingState /> : error ? <ErrorState message={error} /> : rows.length ? rows.map((row) => <DataCard key={row.id} title={row.code} meta={`${row.typeName} · ${row.locationCode || "—"}`} status={row.lifecycleStatusCode}>
...
export function ExceptionsScreen({ api, onBack }: { api: WarehouseMobileApi; onBack: () => void }) {
  const [query, setQuery] = useState("")
  ...
  const load = useCallback(async () => { setLoading(true); setError(null); try { setRows(await api.listExceptions({ search: query.trim() })) } catch (loadError) { setError(message(loadError)) } finally { setLoading(false) } }, [api, query])
  useEffect(() => { void load() }, []) // eslint-disable-line react-hooks/exhaustive-deps
  async function resolveDataError() {
    if (!selected || !notes.trim()) return
    setBusy(true); setError(null)
    try {
      await api.resolveLocationDataError({ facilityId: selected.facilityId, exceptionId: selected.id, notes: notes.trim() })
  ...
  return <WarehouseScreen title={wt("exceptions")} subtitle={wt("exceptionsDetail")} onBack={onBack} actions={<WarehouseButton label={wt("refresh")} tone="secondary" onPress={() => void load()} />}>
    <ScanField value={query} onChangeText={setQuery} onSubmit={() => void load()} />
    {success ? <SuccessState message={success} /> : null}
    {loading ? <LoadingState /> : error ? <ErrorState message={error} /> : rows.length ? rows.map((row) => <DataCard key={row.id} title={row.title} meta={`${row.expectedLocationCode || "—"} · ${new Date(row.raisedAt).toLocaleString()}`} status={`${row.severityCode} · ${row.statusCode}`} onPress={row.typeCode === "location_empty" ? () => { setSelected(row); setSuccess(null) } : undefined}>
      {row.description ? <Text style={styles.detail}>{row.description}</Text> : null}
    </DataCard>) : <EmptyState />}
    {selected ? <>
      <ErrorState message={wt("resolveWarning")} />
