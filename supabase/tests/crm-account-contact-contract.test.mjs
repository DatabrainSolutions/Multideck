...
const addressRoleMigration = readFileSync(new URL("../migrations/20260910161036_add_accounts_receivable_and_operations_address_roles.sql", import.meta.url), "utf8")
...
test("company foundations preserve offices, address capabilities, defaults, and contact history", () => {
  ...
  assert.match(accountDetail, /view="addresses"/)
  assert.match(accountOperations, /label: t\("Details"\)/)
  assert.match(organisationFoundation, /role="button" tabIndex=\{0\} aria-selected=\{selected\}/)
  assert.match(organisationFoundation, /Select another table row to change the address shown here/)
  assert.match(organisationFoundation, /Find an address online/)
  assert.match(organisationFoundation, /Choose country first/)
  assert.match(organisationFoundation, /completeCountryOptions/)
  assert.match(organisationFoundation, /addressFieldsForCountry/)
  assert.match(organisationFoundation, /Address purposes and defaults/)
  assert.match(organisationFoundation, /lg:grid-cols-\[minmax\(0,3fr\)_minmax\(270px,2fr\)\]/)
  assert.match(organisationFoundation, /function InlineField/)
  assert.match(organisationFoundation, /divide-y divide-\[var\(--md-line\)\]/)
  assert.match(organisationFoundation, /selected\?\.isDefault/)
  assert.match(customers, /GOOGLE_PLACES_API_KEY/)
  assert.match(customers, /includedRegionCodes/)
  assert.match(addressRoleMigration, /'accounts_receivable', 'Accounts receivable'/)
  assert.match(addressRoleMigration, /'operations', 'Operations'/)
...
test("selecting a linked account contact opens its contact record accessibly", () => {
  assert.match(accountDetail, /onOpenContact=\{\(contactId\) => navigate\(`\/crm\/contacts\/\$\{contactId\}`\)\}/)
  assert.match(accountOperations, /role="link"/)
  assert.match(accountOperations, /tabIndex=\{0\}/)
  assert.match(accountOperations, /event\.key === "Enter" \|\| event\.key === " "/)
  assert.match(accountOperations, /onOpenContact\(contact\.id\)/)
})
