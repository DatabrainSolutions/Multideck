...
test("the client restores and saves the profile appearance through Supabase RPCs", () => {
  ...
  assert.match(themeProfileSync, /const pendingMode = pendingThemeIntent\.current[\s\S]*?saveTheme\(pendingMode, userId\)/)
  assert.match(themeProfileSync, /lastPersistedTheme\.current = mode[\s\S]*?pendingThemeIntent\.current = null/)
  assert.doesNotMatch(themeProfileSync, /if \(pendingThemeIntent\.current === mode\) \{\s*pendingThemeIntent\.current = null/)
  assert.match(themeProfileSync, /setThemeRef\.current = setTheme/)
  assert.match(themeProfileSync, /setThemeRef\.current\(savedTheme\)/)
  assert.doesNotMatch(themeProfileSync, /\}, \[saveTheme, setTheme\]\)/)
