...
test("the client restores and saves the profile appearance through Supabase RPCs", () => {
  ...
  assert.match(themeProfileSync, /updateWorkspaceBootstrapPreferences\(\{ themeMode: mode \}\)/)
  assert.match(app, /<ThemeProvider[\s\S]*?disableTransitionOnChange[\s\S]*?<ThemeProfileSync \/>/)
  assert.match(themeProfileSync, /localChoice = \{ mode, at: performance\.now\(\) \}/)
  assert.match(themeProfileSync, /const startedAt = performance\.now\(\)[\s\S]*?client\.auth\.getSession\(\)/)
  assert.match(themeProfileSync, /const isStaleRead = localChoice && localChoice\.at > startedAt/)
  assert.match(themeProfileSync, /const wasUnwrittenAtStart = choiceAtStart && persistedThemeAtStart !== choiceAtStart\.mode/)
  assert.match(themeProfileSync, /const isUnwritten = localChoice && lastPersistedTheme !== localChoice\.mode/)
  assert.match(themeProfileSync, /if \(localChoice && \(isStaleRead \|\| wasUnwrittenAtStart \|\| isUnwritten\)\)/)
  assert.match(themeProfileSync, /if \(appliedMode\(\) !== savedTheme\) setTheme\(savedTheme\)/)
