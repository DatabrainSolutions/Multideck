...
test("customs invoice extraction remains authenticated, private, versioned and Mistral-only", async () => {
  ...
  assert.match(edge, /rpc\("customs_declaration_authorised"/)
  assert.match(edge, /caller_auth_user_id: actor\.authUserId/)
  ...
  assert.match(edge, /createSignedUrl\(stored\.DOCStoredObject_BlobName, signedUrlLifetimeSeconds\)/)
  assert.match(edge, /const document = invoiceOcrDocument\(pdfBytes\)/)
  ...
  assert.doesNotMatch(edge, /document_url: signedUrl/)
  assert.match(edge, /Server-Timing/)
  assert.match(shared, /MISTRAL_OCR_MODEL = "mistral-ocr-latest"/)
  assert.match(shared, /COMMERCIAL_INVOICE_SCHEMA_VERSION = 4/)
...
test("purchase order PDFs reuse the authenticated cached extraction boundary and require operator review", async () => {
  ...
  assert.match(workspace, /Match every line to a warehouse item before confirming it for goods in/)
  assert.match(workspace, /Review every field before saving the expected receipt/)
  assert.match(workspace, /reviewedAt:new Date\(\)\.toISOString\(\)/)
...
test("the client uploads immediately, restores server results and applies reviewed lines", async () => {
  ...
  assert.match(workspace, /document\.addEventListener\("keydown", trapFocus, true\)/)
  assert.match(workspace, /document\.addEventListener\("focusin", keepFocusInside, true\)/)
  ...
  assert.match(workspace, /Import invoice header/)
  assert.match(workspace, /Import invoice items/)
