...
let document, submits = 0, identities = 0, accountCompany = 'Example', accountType = 'Expense Account', lostSubmit = false
const input = { id: 'journal-1', company: 'Example', currency: 'GBP', date: '2026-09-18', description: 'Accrual', lines: [{ account: 'Cost', debit: '100.1234', credit: '0', description: '' }, { account: 'Accrual', debit: '0', credit: '100.1234', description: '' }] }
globalThis.__journalTest = {
  async list(type) { return type === 'Company' ? [{ name: 'Example', default_currency: 'GBP' }] : [{ company: accountCompany, account_currency: 'GBP', is_group: 0, disabled: 0, account_type: accountType }] },
...
test('wrong company, control accounts and imbalance block before provider creation', async () => {
  ...
  accountType = 'Expense Account'; await assert.rejects(deliver({ ...input, lines: [{ ...input.lines[0], debit: '101' }, input.lines[1]] }), /balance/)
