...
test("the branded invite opens password creation without a consumable auth link or code", () => {
  ...
  assert.match(authFlow, /getSupabaseSession\(\)/)
  ...
  assert.match(authFlow, /minLength=\{PASSWORD_MIN_LENGTH\}/)
  assert.match(authFlow, /getPasswordPolicyError\(password\)/)
  assert.match(acceptInvitation, /getPasswordPolicyError\(password\)/)
