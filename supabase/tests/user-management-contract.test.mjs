...
test("the branded invite explicitly hands users to password creation", () => {
  ...
  assert.match(authEmail, /confirmationUrl\.searchParams\.set\("token_hash", tokenHash\)/)
  assert.match(authFlow, /supabase\.auth\.verifyOtp\(/)
  assert.match(authFlow, /Confirm your invitation/)
  assert.match(authFlow, /Open the most recent Multideck invitation in your inbox\./)
