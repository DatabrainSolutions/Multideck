...
test("metadata triage is cheap and invoice content uses a high-recall freight relevance gate", () => {
  ...
  assert.match(worker, /A filename or subject containing only the word invoice is not proof of freight relevance/)
  assert.match(worker, /Missing a genuine freight document is worse than sending an uncertain document for human review/)
  assert.match(worker, /classification\.type === "commercial_invoice" && highConfidenceIrrelevantInvoice\(extracted\)/)
  assert.match(worker, /AIInboxJob_ClassificationMethod: "content_irrelevant"/)
  assert.match(worker, /Number\(extracted\.relevanceConfidence\) >= IRRELEVANT_FREIGHT_CONFIDENCE/)
  assert.match(worker, /const IRRELEVANT_FREIGHT_CONFIDENCE = 0\.9/)
  assert.match(worker, /const RELEVANCE_VERSION = "freight-relevance-v1"/)
