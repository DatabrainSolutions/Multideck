...
// Direct private-adapter contract only. Registry, approval and watch lifecycle
// coverage must be added before this capability can be released.
export const securityEvidenceDexterFixture=migration+`
...
begin
  ...
  result:=public.multideck_dexter_action_record_booking_security_evidence(company,actor,proposal);
  record_id:=(result->>'recordId')::uuid;
  source:=public.multideck_dexter_domain_booking_security_evidence(company,record_id::text,1)->0;
