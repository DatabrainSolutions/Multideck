...
test("Dexter redirects off-topic requests without narrowing useful freight work", () => {
  ...
  assert.match(edgeFunction, /const tools = \[\.\.\.scopeBoundaryTools\(\), \.\.\.pendingApprovalTools, \.\.\.\(accessMode === "approve" \? \[deferredWorkTool\] : \[\]\), recordTableTool, \.\.\.readTools, \.\.\.documentTools, \.\.\.emailTools, \.\.\.writingTools, \.\.\.actionTools\]/)
  assert.equal(edgeFunction.match(/call\.name === DEXTER_SCOPE_REDIRECT_TOOL/g)?.length, 1)
...
test("Customs declarations are connected to Dexter with explicit import/export provider filing controls", () => {
  ...
  assert.match(edgeFunction, /It always prepares one exact submission for review and waits for explicit approval/)
  assert.equal(requiresExplicitActionApproval("submit_customs_declaration", "full"), true)
  ...
  assert.match(edgeFunction, /\["customs_declarations", "customs_calculations", "customs_assessments"\]\.includes\(domain\)/)
...
test("Dexter watches compile once and evaluate owner-private source changes without LLM polling", () => {
  ...
  assert.match(notificationEmailFunction, /\(eventType === "dexter_watch" \|\| eventType === "document_parse"\) && preference\?\.CommNotifPref_IsEnabled !== true/)
  ...
  assert.match(dexterPage, /dexterMode === "watch"/)
  assert.match(dexterPage, /createDexterWatch/)
...
test("untrusted Home email subjects cannot turn a suggestion into an automatic send", () => {
  ...
  assert.equal(requiresExplicitActionApproval("create_email_draft", "full"), true)
...
test("Dexter reads PDFs directly and retains Mistral conversion before approval-safe writes", () => {
  ...
  assert.equal(edgeFunction.match(/call\.name === DEXTER_DOCUMENT_OCR_TOOL/g)?.length, 1)
  assert.match(edgeFunction, /Read attached PDF\/image inputs directly when present/)
  assert.match(edgeFunction, /call \$\{DEXTER_DOCUMENT_OCR_TOOL\} before answering from their contents or preparing a write/)
...
test("approval preparation stays visible and carries field-level before and after data", () => {
  assert.match(dexterPage, /isStreamingMessage/)
  assert.match(dexterPage, /Thinking…/)
  ...
  assert.match(edgeFunction, /rememberCurrentRecords\(toolOutput, currentRecordsById\)/)
...
test("Dexter responses keep structured Markdown and one stable reasoning disclosure", () => {
  ...
  assert.match(edgeFunction, /use show_record_table after querying records/)
  assert.match(read("supabase/functions/agent-dexter/record-tables.ts"), /column\("companyName", "Lead"\)/)
  ...
  assert.match(dexterPage, /<DexterActivityTrail/)
  assert.match(read("multideck.client/src/components/multideck/dexter-activity-trail.tsx"), /data-reasoning-state=/)
...
test("Gmail and Outlook reads use connected provider tools while writes require approval", () => {
  ...
  assert.match(edgeFunction, /const searchableEmailProviders: DexterEmailProvider\[\] = emailEnabled \? \["gmail", "outlook"\] : \[\]/)
  assert.match(edgeFunction, /emailProvidersForReferences\(retainedEmailReferences\)/)
  assert.match(edgeFunction, /\.\.\.directEmailReferences, \.\.\.previousEmailAttachments/)
