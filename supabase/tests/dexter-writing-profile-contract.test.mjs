...
test("Approve mode keeps email actions inline while Full access executes the same allowlisted action", () => {
  ...
  assert.match(dexter, /securePreparedEmailAction/)
  assert.match(dexter, /EMAIL_PREPARED_ACTIONS[\s\S]*claimExternalPreparedAction/)
  assert.match(dexter, /executePreparedActionById/)
