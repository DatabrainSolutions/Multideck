...
test("Dexter injects style tooling only for explicit email writing and treats it as tone only", () => {
  ...
  assert.match(dexter, /const writingEmailObject =/)
...
test("Approve mode keeps email actions inline while Full access executes the same allowlisted action", () => {
  ...
  assert.match(dexter, /requestedAction === "send" \? body\.draftMessageId \? "\/provider-drafts\/send" : "\/send" : "\/provider-drafts"/)
...
test("settings and the component catalogue expose the finished product surface", () => {
  ...
  assert.match(settings, /profileNotice/)
  assert.match(settings, /profileUpdatedLabel/)
