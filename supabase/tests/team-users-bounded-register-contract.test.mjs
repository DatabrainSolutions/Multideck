...
test("desktop and mobile Users views share debounced server paging, sorting and one photo-signing batch", () => {
  ...
  assert.match(settingsPage, /pagination=\{\{ offset: userOffset, limit: userPageSize, total: totalUsers/)
  assert.match(settingsPage, /onLimitChange: setUserPageSize/)
  assert.match(settingsPage, /<Pagination[\s\S]*pageCount=\{Math\.max\(1, Math\.ceil\(totalUsers \/ userPageSize\)\)\}/)
