...
const built = await build({
  stdin: { contents: `
    ...
    if(routingScenario){
      ...
      initial.differences=[{key:'routing',label:'Routing plan',section:'Route & service',previousQuoteValue:[route,{...route,mode:'Road'}],bookingValue:[route,{...route,mode:'Road'}],newQuoteValue:[{...route,mode:routingScenario==='dates'?'Sea':'Air',plannedArrivalAt:'2026-10-03'},{...route,mode:'Road'}],requiresConfirmation:routingScenario!=='dates',warningCode:routingScenario==='dates'?null:'mode_change',recommendation:routingScenario==='dates'?'apply':'review',reviewNote:'Review each Quote-owned leg; Booking-only legs are retained.'}];
    }
