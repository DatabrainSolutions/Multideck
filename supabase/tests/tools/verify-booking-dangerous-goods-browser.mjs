...
const start=source.indexOf('export function BookingDangerousGoodsPreview()'),end=source.indexOf('export function BookingRouteMilestonesPreview()',start)
...
const built=await build({stdin:{contents:`
  ...
  createRoot(document.getElementById('root')).render(<main className="min-w-0 p-4 text-[13px]"><BookingDangerousGoodsPreview/></main>);
`,loader:'tsx',resolveDir:client},bundle:true,write:false,format:'esm',jsx:'automatic',alias:{'@':`${client}src`},plugins:[{name:'isolated-boundaries',setup(b){
  ...
  b.onLoad({filter:/.*/,namespace:'qa'},({path})=>({contents:path==='api'
    ? `export async function saveBookingDangerousGoods(){throw Error('Tenant API must never run in this fixture')}`
...
const server=createServer((req,res)=>{
  ...
  res.setHeader('Content-Type','text/html');res.end('<!doctype html><html lang="en-GB"><head><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="/style.css"><title>Dangerous-goods isolated QA</title></head><body><div id="root"></div><script type="module" src="/app.js"></script></body></html>')
})
...
try{
  ...
  for(const language of ['en-GB','en-US'])for(const motion of ['reduce','no-preference']){
    ...
    const open=page.getByRole('button',{name:'Record dangerous goods',exact:true})
    await open.focus();await page.keyboard.press('Enter')
    const dialog=page.getByRole('dialog'),un=page.getByRole('textbox',{name:'UN number',exact:true})
    await un.waitFor()
    await page.waitForFunction(()=>document.activeElement?.getAttribute('name')==='unNumber')
    await page.keyboard.press('Shift+Tab')
    assert.equal(await page.evaluate(()=>document.activeElement?.textContent),'Close')
    await page.keyboard.press('Tab')
    assert.equal(await page.evaluate(()=>document.activeElement?.getAttribute('name')),'unNumber')
    ...
    await page.waitForFunction(()=>document.activeElement?.textContent==='Record dangerous goods')
    assert.equal(await page.getByText('1234',{exact:true}).count()>0,true)
