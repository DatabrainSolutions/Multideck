...
test('warehouse pricing persists audited cards, rejects stale/invalid writes and protects colleague, customer and company boundaries', () => {
  ...
  try {
    ...
    run('psql', ['-h', dir, '-U', 'postgres', '-v', 'ON_ERROR_STOP=1'], `
      ...
      reset role;
      do $$begin if (select count(*) from booking_api.warehouse_pricing_audit)<>2 then raise exception 'audit not atomic'; end if; end$$;
      ...
      set role authenticated;
      do $$begin if public.warehouse_pricing_card()->>'version'<>'1' then raise exception 'inactive creator hides rates'; end if; end$$;
      reset role;
