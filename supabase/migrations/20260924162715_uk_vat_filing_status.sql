...
-- A bounded, read-only operator projection. Never return the OAuth tokens,
-- submission body, raw HMRC response, tenant project identifier or software-only
-- period key.
create function public.multideck_uk_vat_filing_status(
...
begin
  ...
  order by coalesce(observation.id=v_approval.obligation_verification_id,false) desc,
...
revoke all on function public.multideck_uk_vat_filing_status(uuid,uuid,uuid)
  from public,anon,authenticated;
grant execute on function public.multideck_uk_vat_filing_status(uuid,uuid,uuid)
  to service_role;
