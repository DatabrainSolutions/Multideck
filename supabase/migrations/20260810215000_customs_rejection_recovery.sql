...
begin
  ...
  where "CUST_id" = p_declaration_id
    ...
    and "CUST_DeclarationKind" in ('cds_export', 'cds_import')
    and "CUST_Status" = 'rejected'
    and not "CUST_IsDeleted"
  returning "CUST_Status" into v_status;
  ...
  insert into public."Customs_AuditLog" (
    ...
    p_declaration_id,
    'customs_rejection_correction_started',
    ...
    'multideck_app',
    'The operator started a corrected declaration after a Customs rejection.'
  );
...
comment on function public.reopen_rejected_customs_declaration(uuid) is
  'Reopens an operator-owned rejected CDS declaration for correction. Submitted and accepted declarations remain immutable.';
