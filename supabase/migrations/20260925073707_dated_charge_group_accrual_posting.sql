...
-- Month-end charge estimates use the mapping that was independently approved
-- for the period end, just as invoice approval uses its accounting date.
-- Editable mapping proposals cannot silently change an approved close.
...
declare
  ...
  v_cutover public."FIN_ChargeMappingCutovers"%rowtype;
...
begin
  ...
  if v_new_chart then
    select * into v_cutover from public."FIN_ChargeMappingCutovers"
      where legal_entity_id=v_run."FINCloseRun_LegalEntityID" and status='active'
        and effective_date<=v_period."FINPeriod_EndDate"
      order by effective_date desc limit 1 for share;
    if not found then
      raise exception 'Activate a reviewed charge mapping cutover for this period before accrual posting.' using errcode='22023';
    end if;
  end if;
  ...
  if v_has_charge_rows then
    ...
    loop
      if v_new_chart then
        ...
        v_mapped:=v_cutover.mapping_snapshot->v_charge_code_id::text;
        if v_mapped is null then raise exception 'Map every charge code in the active dated cutover before accrual posting.' using errcode='22023'; end if;
        if v_charge."FINChargePeriod_ApprovedAccrual">0 then
          if v_mapped#>>'{cost,id}' is null then raise exception 'Map this charge code to a cost nominal group.' using errcode='22023'; end if;
          v_group:=public._multideck_validate_nominal_group(v_run."FINCloseRun_LegalEntityID",(v_mapped#>>'{cost,id}')::uuid,'cost');
          if v_group is distinct from v_mapped->'cost' then raise exception 'The approved cost group changed; propose a new dated cutover.' using errcode='22023'; end if;
        ...
        if v_charge."FINChargePeriod_ApprovedWIP">0 then
          if v_mapped#>>'{revenue,id}' is null then raise exception 'Map this charge code to a revenue nominal group.' using errcode='22023'; end if;
          v_group:=public._multideck_validate_nominal_group(v_run."FINCloseRun_LegalEntityID",(v_mapped#>>'{revenue,id}')::uuid,'revenue');
          if v_group is distinct from v_mapped->'revenue' then raise exception 'The approved revenue group changed; propose a new dated cutover.' using errcode='22023'; end if;
