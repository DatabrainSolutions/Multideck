...
-- Source-only preview for the first VAT period after a posted six-month
-- repayment that contains a later supplier payment. It makes no VAT entry.
create function public.multideck_uk_vat_later_input_tax_restoration_source(
...
begin
  ...
  select coalesce(sum(allocation."FINCashAlloc_AllocatedAmount") filter (where
    ...
    where allocation."FINCashAlloc_DocumentID"=p_document
      and allocation."FINCashAlloc_AllocationStatusCode"='allocated';
  if v_paid_before is distinct from v_gross-v_first.unpaid_at_period_end
  ...
  v_previous_target:=-v_first.box4_delta_gbp;
  ...
  if v_running_paid<>v_paid_end or jsonb_array_length(v_payments)=0
    or v_total<0
    or v_total<>-v_first.box4_delta_gbp-v_previous_target then
  ...
  return jsonb_build_object('periodId',p_period,'documentId',p_document,
    ...
    'priorRepaymentOutstandingGbp',-v_first.box4_delta_gbp,
