...
begin
  ...
  select coalesce(jsonb_agg(value),'[]') into differences from (
...
begin
  ...
  select review.* into review_row from booking_api.quote_sync_reviews review join public."Job_Header" job on job."Job_ID"=review.job_id
...
begin
  ...
  select * into strict review_row from booking_api.quote_sync_reviews where review_id=requested_review_id and job_id=requested_job_id
    and company_id=app_user."Company_ID" and status_code in ('pending','partially_applied','applied') for update;
  ...
  if jsonb_array_length(selected)=0 then
    return jsonb_build_object('reviewId',requested_review_id,'reused',true,'appliedFields',review_row.applied_fields,
  ...
  return coalesce(result,'{}')||jsonb_build_object('reviewId',requested_review_id,'workspace',booking_api.workspace_with_document_groups(caller_auth_user_id,job_row."Job_BookingReference"));
