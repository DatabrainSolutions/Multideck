-- Search a user's Dexter conversation titles and every saved message without
-- exposing another user or tenant's chat content to the browser.
    ...
    select
      jsonb_build_object(
        ...
        'matchSnippet', coalesce(
          (
            select left(regexp_replace(message."AIMSG_Content", '[[:space:]]+', ' ', 'g'), 240)
            from public."AI_Messages" message
            where message."AIMSG_ConversationID" = conversation."AICNV_ID"
              and position(v_query in lower(coalesce(message."AIMSG_Content", ''))) > 0
