...
alter table public.mileage_trips add column company_name text;

update public.mileage_trips trip
set company_name=organisation."Org_Name"
from public."Org_Master" organisation
where trip.account_id=organisation."Org_id";

alter table public.mileage_trips
  add constraint mileage_trips_company_name_length
...
begin
 ...
 if p_action='context' then
   ...
   insert into public.mileage_trips select t.* on conflict(id) do update set
    account_id=excluded.account_id,company_name=excluded.company_name,trip_date=excluded.trip_date,purpose=excluded.purpose,origin=excluded.origin,destination=excluded.destination,waypoints=excluded.waypoints,round_trip=excluded.round_trip,
...
create or replace function public.multideck_dexter_domain_mileage(p_company_id uuid,p_search text,p_take integer)
...
begin
 ...
 select coalesce(jsonb_agg(evidence),'[]') into result from (
   select jsonb_build_object('recordId',id,'employee',employee_name,'tripDate',trip_date,'purpose',purpose,'company',company_name,'miles',distance_miles,
   ...
   and (nullif(btrim(p_search),'') is null or id::text=p_search or concat_ws(' ',employee_name,company_name,purpose,status) ilike '%'||left(p_search,100)||'%')
