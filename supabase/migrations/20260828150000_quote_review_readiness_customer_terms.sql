-- Align quote review readiness with the current Quote details screen.
-- Collection is the visible field under Shipper. Terms are inherited from
-- the selected customer organisation when the saved quote value is blank.
-- Subject-to-rate / space is guidance text and is intentionally optional.
...
begin
  select quote.*,
    coalesce(
      nullif(btrim((
        select profile."CRMAccount_MetadataJSON"->'quoteTerms'->>'terms'
        from public."CRM_AccountProfiles" profile
        where profile."CRMAccount_OrgID" = quote."CusQuoteHeader_CustomerID"
          and not profile."CRMAccount_IsDeleted"
        limit 1
      )), ''),
      nullif(btrim(quote."CusQuoteHeader_TermsText"), '')
    ) as effective_terms
  into quote_row
  ...
  -- This is the visible “Collection” field in the Shipper section. It is
  -- required only when the selected terms/service means collection is part
  -- of the quoted movement.
  if incoterm_value = 'EXW' or lower(coalesce(facts->>'collectionRequired', 'false')) in ('true','yes','1')
     or service_value like '%door%' then
    if nullif(btrim(quote_row."CusQuoteHeader_CollectionAddress"), '') is null then missing := array_append(missing, 'Collection'); end if;
  end if;
  ...
  if nullif(btrim(quote_row.effective_terms), '') is null then missing := array_append(missing, 'Terms and conditions'); end if;

  -- Subject-to-rate / space is optional guidance and must not block sending.
  if not exists (
