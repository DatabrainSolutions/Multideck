-- Reinviting a revoked warehouse customer must reactivate the historical role
-- assignment instead of inserting the same user/role/site/organisation key.
-- This also makes changing back to a previously held warehouse role safe.
-- Dexter parity: this repairs the existing warehouse-user mutation transaction;
-- invitations remain an explicitly unsupported Dexter/Watching write action.
