-- Tenant-local Phone calls workers. Provider credentials remain owned by the
-- Edge Function; cron receives only the shared worker credential from Vault.
...
comment on function public.multideck_phone_call_configure_worker_schedules() is
  'Idempotently configures tenant-local Phone calls workers after validating Vault configuration. Provider polling is opt-in through provider-ready Vault markers.';
