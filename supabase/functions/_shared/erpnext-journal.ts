...
export type JournalExport = { id: string; siteOrigin: string; company: string; currency: string; date: string; description: string; lines: Array<{ account: string; nominalCode: string; expectedRootType: string; debit: string; credit: string; description: string }> }
...
export async function validateErpNextJournal(input: JournalExport) {
  ...
  for (const line of input.lines) {
    if (!line.nominalCode || !["Asset", "Liability", "Equity", "Income", "Expense"].includes(line.expectedRootType)) throw new HttpError(409, "This journal's frozen delivery predates verified nominal mappings. Review its external identity before retrying.")
    const account = line.account
    const matches = await erpNextList("Account", ["name", "company", "account_currency", "is_group", "disabled", "account_type", "root_type"], [["name", "=", account]])
    const row = matches[0]
    ...
    if (row.root_type !== line.expectedRootType) throw new HttpError(409, `Nominal ${line.nominalCode} is mapped to an ERPNext ${row.root_type ?? "unclassified"} account; it requires ${line.expectedRootType}. Review Finance Admin → Mappings.`)
  }
}
