...
function incompleteEvidence(snapshot: any, connection: any, cutoff: string, mappingRevision: string): PeriodEvidence {
  const domains = Object.fromEntries(PERIOD_DOMAINS.map(domain => [domain, { complete: false, pages: 0, count: 0, hash: "0".repeat(64), rows: [] }])) as unknown as PeriodEvidence["domains"]
...
export async function runFinancePeriodComparison(admin: any, actorId: string, entityId: string, periodId: string, connection: any) {
  ...
  let local: PeriodEvidence = incompleteEvidence(second, connection, cutoff, mappingRevision)
  let provider: PeriodEvidence = incompleteEvidence(second, connection, cutoff, mappingRevision)
