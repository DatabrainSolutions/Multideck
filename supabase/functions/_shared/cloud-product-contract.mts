...
export type FeatureRequest = { contractVersion: 1; action: 'features'; tenantId: string; revision: number; features: string[] };
...
export function parseProductRequest(input: unknown, configuredTenantId: string): FeatureRequest | HealthRequest {
  ...
  if (value.action !== 'features' || !Number.isSafeInteger(value.revision) || Number(value.revision) < 1
    || !Array.isArray(value.features) || value.features.length > 1 || value.features.some(f => f !== 'icustoms')) {
    throw new ProductRequestError(400, 'Invalid feature request. Only iCustoms is purchasable.');
  }
  return { contractVersion: 1, action: 'features', tenantId: configuredTenantId, revision: Number(value.revision), features: [...value.features] };
}
