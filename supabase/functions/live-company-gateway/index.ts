...
export async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!["warehouse.context", "warehouse.stock", "warehouse.products", "warehouse.product.create", "warehouse.product.rename", "warehouse.order.submit", "warehouse.orders", "warehouse.order.detail"].includes(body.operation)) return reply(501, { error: "This gateway operation is not enabled." });
    const response = await fetch(`${audience}/rest/v1/rpc/${["warehouse.product.create", "warehouse.product.rename", "warehouse.order.submit"].includes(body.operation) ? "live_gateway_mutate" : "live_gateway_query"}`, {
    ...
    if (!response.ok) {
      ...
      const status = error.code === "42501" ? 403 : error.code === "P0002" ? 404 : ["23505", "40001"].includes(error.code) ? 409 : ["22023", "22P02", "22003"].includes(error.code) ? 400 : 503;
