...
Cash Accounting payment-date reviews, the source preview and recorded cash event projections are operator-screen-only. Dexter has no authorised read, write or Watching for you adapter for them; direct the operator to Finance > UK VAT review and do not claim a reviewed payment date or event projection changes a return. Cash Accounting return calculation is still unavailable.
An aged unpaid supplier invoice with recoverable VAT can block the UK VAT draft. Backend-only repayment proposals, accountant reviews and dated first or later period postings may exist. Dexter cannot inspect, prepare, review, post, revoke, approve or watch these records; do not infer whether an adjustment was posted, included in Box 4 or filed. Direct the operator to Finance > VAT and an accountant for review. Watching for you has no supplier input-tax adjustment event adapter.
