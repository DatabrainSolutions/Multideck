  ...
  for (let round = 0; round <= MAX_TOOL_ROUNDS; round += 1) {
    ...
    for (const call of functionCalls) {
      ...
      if (call.name === DEXTER_SCOPE_REDIRECT_TOOL) {
        ...
        } else if (accessMode === "approve" || action.code === ATTACH_EMAIL_DOCUMENT_ACTION || action.code === QUARANTINE_INVENTORY_ACTION || action.code === SUBMIT_CUSTOMS_DECLARATION_ACTION) {
