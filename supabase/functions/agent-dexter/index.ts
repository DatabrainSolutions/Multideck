...
Operational Booking charge-line inspection, edits and individual accepted Quote charge decisions are not exposed through Dexter yet. Direct the operator to Booking Finance and its charge review. Never use generic Booking updates or Finance draft actions to replace these charges. Removed charges require an explicit reviewed restoration; Provisional discarded charges must never return on reopening. Individual charge-change and Quote charge-decision watches are unsupported; explain this instead of substituting status, headline value or updatedAt watches.
Accepted Quote PDF and Booking invoice/packing-list binary retrieval and private download links are not exposed through Dexter chat. Saving a Multideck Customs draft does not create or update an iCustoms draft; provider-draft creation/update remains a separate explicit action. Job-related declaration source invoices and packing lists can be viewed/downloaded in Customs > Documents & offices (Import terms for imports) or Invoice header > Source documents, and in Booking > Documents. These source-file links and binary reads are also unsupported in chat; source-document-open/download watches are unsupported. Do not claim that saving a draft performed provider synchronisation. Direct the operator to Booking > Documents > Open PDF for Quotes, or Booking > Customs > View / Download for invoice and packing-list attachments; do not invent a URL or claim to have read the file. PDF-open/download and attachment-open/download activity watches are unsupported because these read-only viewers record no workflow change. Existing Quote acceptance and Booking lifecycle watches remain available.
  ...
  if (operation === "create-watch") {
    ...
    const compilerResult: { response?: JsonObject; status: number; requestId: string } = await requestOpenAI({ admin, companyId: actor.companyId, userId: actor.userId }, openAIKey, {
      ...
      instructions: [
        ...
        "Quote PDF-open/download, Booking attachment-open/download, declaration source-document-open/download and retained-original invoice upload activity watches are unsupported. Explain this and direct the operator to Booking Documents or declaration Source documents; never approximate them with status or updatedAt watches.",
        "Customs readiness-completion watches are unsupported. Do not approximate them with updatedAt or draft status. Existing successful Customs handover watches remain supported.",
