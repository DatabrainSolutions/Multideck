...
async function executePreparedActionById(input: {
  ...
  if (actionCode === "run_screening_check") {
    const freshness = await ensureScreeningList(input.admin)
    if (!freshness.ready) return { data: null, error: { code: "screening_list_unavailable", message: "The UK Sanctions List could not yet be verified. No result was returned. Retry the approved check shortly." } }
  }
...
function addDomainCitations(domain: string, value: unknown) {
  ...
  if (domain === "calendar" && Array.isArray(data)) {
    return {
      ...value,
      data: data.map((record) => {
        if (!isObject(record)) return record
        const recordId = cleanString(record.recordId, 80)
        const title = cleanString(record.title, 240) || "Meeting"
        const route = cleanString(record.route, 500)
        return recordId && route ? addRecordCitation(record, title, route, "Canonical Multideck meeting") : record
      }),
    }
  }

  if (domain === "booking_links" && Array.isArray(data)) {
    return {
      ...value,
      data: data.map((record) => {
        if (!isObject(record)) return record
        const title = cleanString(record.title, 240) || "Booking link"
        return addRecordCitation(record, title, "/calendar/booking-links", "Personal Multideck booking link")
      }),
    }
  }

  if (domain === "external_events" && Array.isArray(data)) {
    return {
      ...value,
      data: data.map((record) => {
        if (!isObject(record)) return record
        const title = cleanString(record.title, 240) || "Busy"
        const route = cleanString(record.route, 500) || "/calendar"
        const source = cleanString(record.source, 20) === "microsoft" ? "Microsoft Calendar" : "Google Calendar"
        return addRecordCitation(record, title, route, `Synced ${source} event mirrored into Multideck`)
      }),
    }
  }
...
# Scope boundary
Organisation integrity repairs are not an available Dexter write action. A missing or ambiguous CRM profile must be reviewed by an administrator against the organisation's existing company, legal entity, financial records and provider mapping. Never infer ownership, create a replacement provider customer, or claim a repair from an existing mapping alone. Chat and Watching for you do not currently expose profile-integrity checks or repair-specific watches; state this limitation explicitly. Existing accounting sync status reads and event-driven watches remain available only when their capabilities are listed.
...
UK VAT transaction reconciliation dates, source locks, credit-to-original links and HMRC filing remain operator-only. Dexter has no direct VAT account detail, write or Watching for you adapter. If asked, say these actions and watches are unsupported in chat and direct the operator to Finance > VAT account audit trail. Never claim to have reconciled, changed, linked or filed a VAT transaction.
...
Cash Accounting payment-date reviews, the source and nine-box previews, and recorded cash event projections are operator-screen-only. Dexter has no authorised read, write or Watching for you adapter for them; direct the operator to Finance > UK VAT review and do not claim a reviewed payment date, event projection or nine-box preview changes a return. Cash Accounting return calculation is still unavailable.
