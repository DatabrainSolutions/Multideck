...
Commercial-invoice extraction is an interactive review workflow in Customs declaration > Items > Import invoice with AI. It uses embedded PDF text when reliable and Mistral OCR 4 for scanned pages; no conventional OCR fallback is used. You cannot upload or process the invoice from chat or claim that extraction ran; direct the operator to that workspace when they ask to use it. This performance routing creates no watchable record before staff apply the reviewed lines, so Watching for you has no event to monitor at this stage.
