...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (parts[0] === "exchange-rates") {
      ...
      const lookup = [...new Set([...codes, "EUR"])]
      const { data: stored, error: rateError } = await admin.rpc("multideck_finance_latest_exchange_rates", { p_currency_codes: lookup })
      if (rateError) {
        if (["42883", "PGRST202"].includes(rateError.code ?? "")) throw new HttpError(503, "Current exchange rates are still being prepared. Try again shortly.")
        throw new HttpError(500, rateError.message)
      }
      const rates = (Array.isArray(stored) ? stored : []).map((item: any) => ({ ...item, provider: item.providerName ? { FINRateProvider_Name: item.providerName } : null }))
